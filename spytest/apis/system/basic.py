...
_DISABLED_COMMANDS = {}
_DISABLED_COMMAND_CACHE_KEY = "system-status-disabled-commands"


def _get_disabled_commands(dut):
    disabled = _DISABLED_COMMANDS.get(dut)
    if disabled is None:
        cached = st.get_cache(_DISABLED_COMMAND_CACHE_KEY, dut, None)
        if isinstance(cached, (list, tuple, set)):
            disabled = set(cached)
        elif isinstance(cached, dict):
            disabled = {key for key, value in cached.items() if value}
        elif cached:
            disabled = {str(cached)}
        else:
            disabled = set()
        _DISABLED_COMMANDS[dut] = disabled
    return disabled


def _persist_disabled_commands(dut, disabled):
    try:
        st.set_cache(_DISABLED_COMMAND_CACHE_KEY, sorted(disabled), dut)
    except Exception:  # pragma: no cover - cache persistence best-effort
        pass


def _disable_command(dut, command):
    disabled = _get_disabled_commands(dut)
    if command not in disabled:
        disabled.add(command)
        _persist_disabled_commands(dut, disabled)


def _is_command_disabled(dut, command):
    return command in _get_disabled_commands(dut)
