"""
OSPF BASIC CONFIGURATION - 4-NODE TOPOLOGY WITH VLAN, STATIC ROUTING, AND REBOOT PERSISTENCE
...
How to run:
  ...
  tests/system/iscli_OSPF/testcases_OSPF_3_iscli_Basic_4_node_Vlan_Reboot.py \
  --logs-path ./logs/testcases_OSPF_3_iscli_Basic_4_node_Vlan_Reboot_$(date +%F_%H%M%S) \
  --log-level debug --skip-init-config --ifname-type native \
  --tc-max-timeout 3600

Description:
  This test validates OSPF configuration with VLAN and static routing in a 4-node topology
  ...
  4. Validating static routes in routing table
  ...
  11. Re-verifying static routes in routing tables after reboot
  12. Re-verifying OSPF neighbor adjacency after reboot
  13. Re-verifying DR/BDR election after reboot
  14. Re-validating ping connectivity from D1 to D3 after reboot
...
# WAIT_AFTER_BGP_RESTART = 180  # Commented out - not needed for reboot test
...
class TestOSPFStaticRoutingVLAN4NodeReboot:
    """Test cases for validating OSPF with VLAN, static routing, and reboot persistence in 4-node topology via CLI (klish mode)."""
    ...
    def setup_class(cls) -> None:
        ...
        st.log("=" * 80)
        st.log("TEST SETUP: Initializing OSPF 4-Node VLAN Static Routing with Reboot Test Suite")
        st.log("=" * 80)
    ...
    def teardown_class(cls) -> None:
        """Cleanup test suite."""
        st.log("=" * 80)
        st.log("TEST TEARDOWN: Cleanup OSPF 4-Node VLAN Static Routing with Reboot Test Suite")
        st.log("=" * 80)
    ...
    # BGP restart section commented out - not needed for reboot test
    # @staticmethod
    ...
    def _verify_ipv4_in_running_config(config_output: str, expected_ipv4: Optional[str]) -> bool:
        ...
        actual_ipv4 = TestOSPFStaticRoutingVLAN4NodeReboot._extract_ipv4_from_running_config(config_output)
    ...
    @pytest.mark.inventory(feature="Regression", testcases=["TC_OSPF_VLAN_STATIC_4NODE_REBOOT_001"])
    @pytest.mark.timeout(3600)
    def test_ospf_vlan_static_routing_4node_reboot_persistence(self) -> None:
        """
        TC_OSPF_VLAN_STATIC_4NODE_REBOOT_001: Validate OSPF with VLAN, static routing, and config persistence across reboot.
        ...
        5. Verify static routes appear in routing tables
        6. Configure OSPF on D2 and D4
        7. Verify OSPF neighbor adjacency (Full state)
        8. Verify DR/BDR election
        9. Verify ping from D1 to D3
        10. Save configuration with 'write memory' on all DUTs
        11. Reboot all devices with 'sudo reboot'
        12. Re-verify static routes in routing tables after reboot
        13. Re-verify OSPF neighbor adjacency after reboot
        14. Re-verify DR/BDR election after reboot
        15. Re-verify ping from D1 to D3 after reboot
        ...
        """
        st.log("\n" + "=" * 80)
        st.log("TEST: OSPF with VLAN, Static Routing, and Reboot Persistence - 4-Node Topology")
        st.log("=" * 80)
        ...
        st.log("PASS: VLANs created and ports added successfully")
        ...
        # ===== STEP 5: Verify static routes in routing tables =====
        st.log("\n" + "-" * 80)
        st.log("STEP 5: Verify static routes in routing tables")
        st.log("-" * 80)
        ...
        # ===== STEP 6: Configure OSPF on D2 and D4 =====
        st.log("\n" + "-" * 80)
        st.log("STEP 6: Configure OSPF on D2 and D4")
        st.log("-" * 80)
        ...
        st.log("\n" + "-" * 80)
        st.log("STEP 7: Verify OSPF neighbor adjacency (Full state)")
        st.log("-" * 80)
        ...
        # ===== STEP 9: Verify ping from D1 to D3 =====
        st.log("\n" + "-" * 80)
        st.log("STEP 9: Verify ping connectivity from D1 to D3")
        st.log("-" * 80)
        ...
        # ===== STEP 10: Save configuration with 'write memory' on all DUTs =====
        st.log("\n" + "-" * 80)
        st.log("STEP 10: Save configuration with 'write memory' on all DUTs")
        st.log("-" * 80)
        ...
        # ===== STEP 11: Reboot all devices with 'sudo reboot' =====
        st.log("\n" + "-" * 80)
        st.log("STEP 11: Reboot all devices with 'sudo reboot'")
        st.log("-" * 80)
        ...
        # Wait for VLANs to stabilize fully before verifying OSPF
        st.log("Waiting 30 seconds for all VLANs to stabilize after reboot...")
        time.sleep(30)

        # ===== STEP 12: Re-verify static routes in routing tables after reboot =====
        st.log("\n" + "-" * 80)
        st.log("STEP 12: Re-verify static routes in routing tables after reboot (config persistence check)")
        st.log("-" * 80)
        ...
        route_output_dut1 = self._get_show_ip_route_output(dut1)
        if not self._verify_static_route_in_routing_table(route_output_dut1, "30.1.1.0/24"):
            error_msg = f"Config persistence failed: Static route 30.1.1.0/24 not found in routing table on {dut1} after reboot"
            st.error(error_msg)
            validation_failures.append(error_msg)
        ...
        route_output_dut3 = self._get_show_ip_route_output(dut3)
        if not self._verify_static_route_in_routing_table(route_output_dut3, "10.1.1.0/24"):
            error_msg = f"Config persistence failed: Static route 10.1.1.0/24 not found in routing table on {dut3} after reboot"
            st.error(error_msg)
            validation_failures.append(error_msg)

        st.log("PASS: Static routes persisted across reboot on D1 and D3")

        # ===== STEP 13: Re-verify OSPF neighbor adjacency after reboot =====
        st.log("\n" + "-" * 80)
        st.log("STEP 13: Re-verify OSPF neighbor adjacency after reboot (config persistence check)")
        st.log("-" * 80)
        ...
        # ===== STEP 14: Re-verify DR/BDR election after reboot =====
        st.log("\n" + "-" * 80)
        st.log("STEP 14: Re-verify DR/BDR election after reboot")
        st.log("-" * 80)
        ...
        # ===== STEP 15: Re-verify ping from D1 to D3 after reboot =====
        st.log("\n" + "-" * 80)
        st.log("STEP 15: Re-verify ping connectivity from D1 to D3 after reboot")
        st.log("-" * 80)
        ...
        # ===== TEST COMPLETE =====
        st.log("\n" + "=" * 80)
        st.log("TEST COMPLETE: OSPF with VLAN, static routing, and reboot persistence validated")
        st.log("Test flow: Remove IPs → VLAN Config → IP Config → Static Routes → OSPF Config → Neighbor Full → DR/BDR → Ping → Save → Reboot All → Verify Persistence → Cleanup ✓")
        st.log("=" * 80)
        ...
        if validation_failures:
            ...
            for dut in [dut1, dut2, dut3, dut4]:
                try:
                    st.generate_tech_support(dut=dut, name="ospf_vlan_reboot_validation_failure")
