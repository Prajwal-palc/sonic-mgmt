"""
OSPF MTU MISMATCH PREVENTS ADJACENCY - 4-NODE TOPOLOGY WITH PORTCHANNEL INTERFACES
...
How to run:
  ...
  tests/system/iscli_OSPF/testcases_OSPF_12_iscli_4_MTU_mismatch_prevents_adj_PC.py \
  --logs-path ./logs/testcases_OSPF_12_iscli_4_MTU_mismatch_prevents_adj_PC_$(date +%F_%H%M%S) \
  --log-level debug --skip-init-config --ifname-type native

Description:
  This test validates that OSPF adjacency is prevented when there is an MTU mismatch
  between neighboring routers on PortChannel interfaces. The test covers:

  1. Remove IP addresses from Ethernet interfaces (baseline)
  2. Create PortChannel and add Ethernet ports as members
  3. Configure IP addresses on PortChannel interfaces with matching MTU
  4. Configure OSPF on both routers
  5. Verify OSPF neighbor adjacency forms successfully (Full state)
  6. Change MTU on one PortChannel to create mismatch
  ...
  Test Scenario:
  - D2 and D4 are connected via PortChannel120 (20.1.1.0/24)
  ...
  - D4 PortChannel MTU is changed to 1500 using 'mtu 1500' command
...
class TestOSPFMTUMismatchPortChannel:
    """Test cases for validating OSPF MTU mismatch prevents adjacency on PortChannel interfaces via CLI (klish mode)."""
    ...
    def setup_class(cls) -> None:
        ...
        st.log("=" * 80)
        st.log("TEST SETUP: Initializing OSPF MTU Mismatch Test Suite (PortChannel)")
        st.log("=" * 80)
    ...
    def teardown_class(cls) -> None:
        """Cleanup test suite."""
        st.log("=" * 80)
        st.log("TEST TEARDOWN: Cleanup OSPF MTU Mismatch Test Suite (PortChannel)")
        st.log("=" * 80)
    ...
    @staticmethod
    def _configure_portchannel_mtu(dut: str, portchannel_id: str, mtu_value: str) -> bool:
        """Configure MTU on PortChannel interface."""
        st.log(f"Configuring MTU {mtu_value} on PortChannel{portchannel_id} on {dut}")
        commands = [
            "configure terminal",
            f"interface PortChannel {portchannel_id}",
            f"mtu {mtu_value}",
    ...
    @staticmethod
    def _get_portchannel_mtu(dut: str, portchannel_id: str) -> Optional[str]:
        """Get current MTU value from PortChannel interface running-configuration."""
        st.log(f"Getting MTU value for PortChannel{portchannel_id} on {dut}")
    ...
    @pytest.mark.inventory(feature="Regression", testcases=["TC_OSPF_MTU_MISMATCH_PC_001"])
    @pytest.mark.negative
    def test_ospf_mtu_mismatch_prevents_adjacency_portchannel(self) -> None:
        """
        TC_OSPF_MTU_MISMATCH_PC_001: Validate that MTU mismatch prevents OSPF adjacency on PortChannel.
        ...
        4. Configure matching MTU (9100) on both PortChannel interfaces
        5. Configure OSPF on both routers
        6. Verify OSPF neighbor adjacency forms (Full state) - baseline
        7. Change MTU on D4 PortChannel to 1500 (create mismatch)
        8. Verify OSPF adjacency is broken or prevented (negative test)
        9. Restore MTU on D4 PortChannel to 9100 (matching)
        ...
        """
        st.log("\n" + "=" * 80)
        st.log("TEST: OSPF MTU Mismatch Prevents Adjacency (PortChannel)")
        st.log("=" * 80)
        ...
        # ===== STEP 4: Configure matching MTU on both PortChannel interfaces =====
        st.log("\n" + "-" * 80)
        st.log(f"STEP 4: Configure matching MTU ({self.data.default_mtu}) on both PortChannel interfaces")
        st.log("-" * 80)

        self._configure_portchannel_mtu(dut2, portchannel_id, self.data.default_mtu)
        self._configure_portchannel_mtu(dut4, portchannel_id, self.data.default_mtu)

        st.log(f"MTU {self.data.default_mtu} configured on both PortChannel interfaces")
        time.sleep(WAIT_AFTER_MTU_CHANGE)

        # Verify MTU configuration
        dut2_mtu = self._get_portchannel_mtu(dut2, portchannel_id)
        dut4_mtu = self._get_portchannel_mtu(dut4, portchannel_id)

        st.log(f"D2 PortChannel{portchannel_id} MTU: {dut2_mtu}")
        st.log(f"D4 PortChannel{portchannel_id} MTU: {dut4_mtu}")
        ...
        # ===== STEP 7: Change MTU on D4 PortChannel to create mismatch =====
        st.log("\n" + "-" * 80)
        st.log(f"STEP 7: Change MTU on D4 PortChannel{portchannel_id} to {self.data.mismatch_mtu} (create mismatch)")
        st.log("-" * 80)

        self._configure_portchannel_mtu(dut4, portchannel_id, self.data.mismatch_mtu)

        st.log(f"MTU on D4 PortChannel{portchannel_id} changed to {self.data.mismatch_mtu}")
        time.sleep(WAIT_AFTER_MTU_CHANGE)

        # Verify MTU mismatch
        dut2_mtu_after = self._get_portchannel_mtu(dut2, portchannel_id)
        dut4_mtu_after = self._get_portchannel_mtu(dut4, portchannel_id)

        st.log(f"MTU mismatch created:")
        st.log(f"  D2 PortChannel{portchannel_id} MTU: {dut2_mtu_after}")
        st.log(f"  D4 PortChannel{portchannel_id} MTU: {dut4_mtu_after}")
        ...
        st.log("\n" + "-" * 80)
        st.log(f"STEP 9: Restore MTU on D4 PortChannel{portchannel_id} to {self.data.default_mtu} (matching)")
        st.log("-" * 80)

        self._configure_portchannel_mtu(dut4, portchannel_id, self.data.default_mtu)

        st.log(f"MTU on D4 PortChannel{portchannel_id} restored to {self.data.default_mtu}")
        time.sleep(WAIT_AFTER_MTU_CHANGE)

        # Verify matching MTU restored
        dut2_mtu_restored = self._get_portchannel_mtu(dut2, portchannel_id)
        dut4_mtu_restored = self._get_portchannel_mtu(dut4, portchannel_id)

        st.log(f"Matching MTU restored:")
        st.log(f"  D2 PortChannel{portchannel_id} MTU: {dut2_mtu_restored}")
        st.log(f"  D4 PortChannel{portchannel_id} MTU: {dut4_mtu_restored}")
        ...
        # ===== TEST COMPLETE =====
        st.log("\n" + "=" * 80)
        st.log("TEST COMPLETE: OSPF MTU mismatch adjacency prevention validated successfully (PortChannel)")
        st.log("Test flow: PortChannel Creation → IP Config → Matching MTU → OSPF Config → Adjacency Full (baseline)")
        ...
        st.log("  1. OSPF adjacency forms with matching MTU (9100) on PortChannel")
        ...
        if validation_failures:
            ...
            try:
                st.generate_tech_support(dut=dut2, name="ospf_mtu_mismatch_pc_validation_failure")
            ...
            try:
                st.generate_tech_support(dut=dut4, name="ospf_mtu_mismatch_pc_validation_failure")
