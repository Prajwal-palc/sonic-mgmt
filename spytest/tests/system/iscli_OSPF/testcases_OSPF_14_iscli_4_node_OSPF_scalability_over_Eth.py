"""
OSPF SCALABILITY TESTING - 4-NODE TOPOLOGY WITH MULTIPLE ETHERNET LINKS
...
How to run:
  ...
  tests/system/iscli_OSPF/testcases_OSPF_14_iscli_4_node_OSPF_scalability_over_Eth.py \
  --logs-path ./logs/testcases_OSPF_14_scalability_$(date +%F_%H%M%S) \
  --log-level debug --skip-init-config --ifname-type native

Description:
  This test validates OSPF scalability in a 4-node topology with multiple parallel
  Ethernet links between devices. The test covers:

  1. Configuring IP addresses on 16 total Ethernet interfaces (4 links between each device pair)
  2. Configuring OSPF Area 0 on all devices and all interfaces
  3. Verifying OSPF neighbor adjacency on all 16 links (Full state)
  4. Verifying OSPF interface parameters (State, DR/BDR election, Router ID, Cost, etc.)
  5. Verifying OSPF routes are learned with ECMP (Equal Cost Multi-Path)
  6. Verifying OSPF database and LSA (Link State Advertisements)
  7. Verifying OSPF neighbor detailed information
  8. Verifying 'show ip ospf border-routers' command output
  9. Testing end-to-end connectivity with ping
  10. Cleanup: Removing all configurations

  Test Focus: OSPF scalability over Ethernet interfaces
  - 4 devices with 4 parallel links between each pair
  - Total of 16 OSPF adjacencies across the topology
  - ECMP load balancing verification
  - DR/BDR election on broadcast networks
  - LSA database validation

  Topology:
        D1 ======== (4 parallel Ethernet links) ======== D2
                                                          ||
                                                          ||
                          (4 parallel Ethernet links)     ||
                                                          ||
        D3 ======== (4 parallel Ethernet links) ======== D4

  Links:
    D1 ↔ D2: Ethernet0,4,8,12 ↔ Ethernet0,4,8,12
    D2 ↔ D4: Ethernet16,20,24,28 ↔ Ethernet16,20,24,28
    D4 ↔ D3: Ethernet32,36,40,44 ↔ Ethernet32,36,40,44

  IP Addressing:
    D1: Ethernet0,4,8,12: 10.1.1.1/24, 10.1.2.1/24, 10.1.3.1/24, 10.1.4.1/24
    D2: Ethernet0,4,8,12: 10.1.1.2/24, 10.1.2.2/24, 10.1.3.2/24, 10.1.4.2/24
        Ethernet16,20,24,28: 20.1.1.1/24, 20.1.2.1/24, 20.1.3.1/24, 20.1.4.1/24
    D4: Ethernet16,20,24,28: 20.1.1.2/24, 20.1.2.2/24, 20.1.3.2/24, 20.1.4.2/24
        Ethernet32,36,40,44: 30.1.1.2/24, 30.1.2.2/24, 30.1.3.2/24, 30.1.4.2/24
    D3: Ethernet32,36,40,44: 30.1.1.1/24, 30.1.2.1/24, 30.1.3.1/24, 30.1.4.1/24
  ...
  'show ip ospf neighbor detail', and 'show ip ospf border-routers' commands
  to validate OSPF scalability. Uses klish CLI type exclusively.
...
class TestOSPFScalabilityOverEthernet:
    """Test cases for validating OSPF scalability over Ethernet links in 4-node topology."""
    ...
    def setup_class(cls) -> None:
        ...
        st.log("=" * 80)
        st.log("TEST SETUP: Initializing OSPF Scalability Test Suite - Ethernet Links")
        st.log("=" * 80)
    ...
    def teardown_class(cls) -> None:
        """Cleanup test suite."""
        st.log("=" * 80)
        st.log("TEST TEARDOWN: Cleanup OSPF Scalability Test Suite")
        st.log("=" * 80)
    ...
    @pytest.mark.inventory(feature="Regression", testcases=["TC_OSPF_SCALABILITY_ETHERNET_001"])
    def test_ospf_scalability_over_ethernet(self) -> None:
        """
        TC_OSPF_SCALABILITY_ETHERNET_001: Validate OSPF scalability over Ethernet links.

        Test Procedure:
        1. Configure IP addresses on all 16 Ethernet interfaces
        2. Configure OSPF on all devices with all networks
        3. Verify OSPF neighbors form on all links (Full state)
        4. Verify OSPF interface status on all devices
        5. Verify DR/BDR election on broadcast networks
        6. Verify OSPF routes are learned with ECMP
        7. Verify OSPF database and LSAs
        8. Verify OSPF neighbor detailed information
        8B. Verify 'show ip ospf border-routers' command output
        9. Verify end-to-end connectivity with ping
        10. Cleanup: Remove all configurations

        Expected Result:
        - IP addresses configured on all 16 interfaces
        - OSPF neighbors form on all links (16 adjacencies total)
        ...
        """
        st.log("\n" + "=" * 80)
        st.log("TEST: OSPF Scalability Over Ethernet - 4-Node Topology with 16 Links")
        st.log("=" * 80)
        ...
        st.log("\n" + "-" * 80)
        st.log("STEP 1: Configure IP addresses on all 16 Ethernet interfaces")
        st.log("-" * 80)
        ...
        st.log("IP addresses configured on all 16 interfaces")
        time.sleep(WAIT_AFTER_IP_CONFIG)

        st.log("PASS: IP address configuration completed")
        ...
        # ===== STEP 3: Verify OSPF neighbors on all devices =====
        st.log("\n" + "-" * 80)
        st.log("STEP 3: Verify OSPF neighbors form on all links (Full state)")
        st.log("-" * 80)
        ...
        if not self._verify_ospf_neighbor_count(d1_neighbor_output, 4):
            error_msg = f"STEP 3: D1 should have 4 OSPF neighbors"
            st.error(error_msg)
        ...
        if not self._verify_ospf_neighbor_count(d2_neighbor_output, 8):
            error_msg = f"STEP 3: D2 should have 8 OSPF neighbors"
            st.error(error_msg)
            validation_failures.append(error_msg)
        else:
        ...
        if not self._verify_ospf_neighbor_count(d3_neighbor_output, 4):
            error_msg = f"STEP 3: D3 should have 4 OSPF neighbors"
            st.error(error_msg)
            validation_failures.append(error_msg)
        else:
        ...
        if not self._verify_ospf_neighbor_count(d4_neighbor_output, 8):
            error_msg = f"STEP 3: D4 should have 8 OSPF neighbors"
            st.error(error_msg)
            validation_failures.append(error_msg)
        else:
        ...
        if len([f for f in validation_failures if "STEP 3" in f]) == 0:
            st.log("PASS: All OSPF neighbors are in Full state - Total 16 adjacencies verified")

        # ===== STEP 4: Verify OSPF interface status =====
        st.log("\n" + "-" * 80)
        st.log("STEP 4: Verify OSPF interface status on all devices")
        st.log("-" * 80)
        ...
        # Verify D1 interfaces
        for interface in self.data.dut1_d2_links:
            if not self._verify_ospf_interface_present(d1_ospf_intf_output, interface):
                error_msg = f"STEP 4: {interface} not present or not up on {dut1}"
                st.error(error_msg)
                validation_failures.append(error_msg)

        # Verify D2 interfaces
        for interface in self.data.dut2_d1_links + self.data.dut2_d4_links:
            if not self._verify_ospf_interface_present(d2_ospf_intf_output, interface):
                error_msg = f"STEP 4: {interface} not present or not up on {dut2}"
                st.error(error_msg)
                validation_failures.append(error_msg)

        # Verify D3 interfaces
        for interface in self.data.dut3_d4_links:
            if not self._verify_ospf_interface_present(d3_ospf_intf_output, interface):
                error_msg = f"STEP 4: {interface} not present or not up on {dut3}"
                st.error(error_msg)
                validation_failures.append(error_msg)

        # Verify D4 interfaces
        for interface in self.data.dut4_d2_links + self.data.dut4_d3_links:
            if not self._verify_ospf_interface_present(d4_ospf_intf_output, interface):
                error_msg = f"STEP 4: {interface} not present or not up on {dut4}"
                st.error(error_msg)
                validation_failures.append(error_msg)

        if len([f for f in validation_failures if "STEP 4" in f]) == 0:
            st.log("PASS: All 16 OSPF interfaces are up and operational")

        # ===== STEP 5: Verify DR/BDR election =====
        st.log("\n" + "-" * 80)
        st.log("STEP 5: Verify DR/BDR election on broadcast networks")
        st.log("-" * 80)
        ...
        # ===== STEP 6: Verify OSPF routes with ECMP =====
        st.log("\n" + "-" * 80)
        st.log("STEP 6: Verify OSPF routes are learned with ECMP")
        st.log("-" * 80)
        ...
        for network in d3_networks:
            if not self._verify_ecmp_routes(d1_route_output, network):
                error_msg = f"STEP 6: D1 missing route to {network}"
                st.error(error_msg)
                validation_failures.append(error_msg)
        ...
        for network in d3_networks:
            if network not in d2_route_output:
                error_msg = f"STEP 6: D2 missing route to {network}"
                st.error(error_msg)
                validation_failures.append(error_msg)
        ...
        for network in d1_networks:
            if not self._verify_ecmp_routes(d3_route_output, network):
                error_msg = f"STEP 6: D3 missing route to {network}"
                st.error(error_msg)
                validation_failures.append(error_msg)
        ...
        # ===== STEP 7: Verify OSPF database and LSAs =====
        st.log("\n" + "-" * 80)
        st.log("STEP 7: Verify OSPF database and LSAs")
        st.log("-" * 80)
        ...
        if not self._verify_lsa_present(d1_db_output, "Router Link States"):
            error_msg = "STEP 7: Router Link States not found in D1 OSPF database"
            st.error(error_msg)
            validation_failures.append(error_msg)

        # Verify Network LSAs present
        if not self._verify_lsa_present(d1_db_output, "Net Link States"):
            error_msg = "STEP 7: Net Link States not found in D1 OSPF database"
            st.error(error_msg)
            validation_failures.append(error_msg)
        ...
        # ===== STEP 8: Verify OSPF neighbor detailed information =====
        st.log("\n" + "-" * 80)
        st.log("STEP 8: Verify OSPF neighbor detailed information")
        st.log("-" * 80)
        ...
        if "State is Full" not in d1_neighbor_detail:
            error_msg = "STEP 8: Neighbor detail missing Full state information on D1"
            st.error(error_msg)
            validation_failures.append(error_msg)
        else:
        ...
        # ===== STEP 8B: Verify OSPF border-routers command =====
        st.log("\n" + "-" * 80)
        st.log("STEP 8B: Verify 'show ip ospf border-routers' command")
        st.log("-" * 80)
        ...
        if not self._verify_border_routers_output(d1_border_routers, single_area_topology=True):
            error_msg = "STEP 8B: 'show ip ospf border-routers' command validation failed on D1"
            st.error(error_msg)
            validation_failures.append(error_msg)

        if not self._verify_border_routers_output(d2_border_routers, single_area_topology=True):
            error_msg = "STEP 8B: 'show ip ospf border-routers' command validation failed on D2"
            st.error(error_msg)
            validation_failures.append(error_msg)

        if not self._verify_border_routers_output(d3_border_routers, single_area_topology=True):
            error_msg = "STEP 8B: 'show ip ospf border-routers' command validation failed on D3"
            st.error(error_msg)
            validation_failures.append(error_msg)

        if not self._verify_border_routers_output(d4_border_routers, single_area_topology=True):
            error_msg = "STEP 8B: 'show ip ospf border-routers' command validation failed on D4"
            st.error(error_msg)
            validation_failures.append(error_msg)

        if len([f for f in validation_failures if "STEP 8B" in f]) == 0:
        ...
        st.log("\n" + "-" * 80)
        st.log("STEP 9: Verify end-to-end connectivity with ping")
        st.log("-" * 80)
        ...
        for ip in ["30.1.1.1", "30.1.2.1", "30.1.3.1", "30.1.4.1"]:
            if not self._verify_ping_success(dut1, ip, 4):
                error_msg = f"STEP 9: Ping from D1 to {ip} (D3) failed"
                st.error(error_msg)
                validation_failures.append(error_msg)
        ...
        for ip in ["10.1.1.1", "10.1.2.1", "10.1.3.1", "10.1.4.1"]:
            if not self._verify_ping_success(dut3, ip, 4):
                error_msg = f"STEP 9: Ping from D3 to {ip} (D1) failed"
                st.error(error_msg)
                validation_failures.append(error_msg)

        # Ping from D2 to D4
        if not self._verify_ping_success(dut2, "30.1.1.1", 4):
            error_msg = f"STEP 9: Ping from D2 to D4's network failed"
            st.error(error_msg)
            validation_failures.append(error_msg)
        ...
        st.log("IP addresses removed from all 16 interfaces")
        ...
        # ===== TEST COMPLETE =====
        st.log("\n" + "=" * 80)
        st.log("TEST COMPLETE: OSPF Scalability Over Ethernet")
        st.log("=" * 80)
        st.log("Summary:")
        st.log("  ✓ 16 Ethernet interfaces configured with IP addresses")
        st.log("  ✓ OSPF configured on all 4 devices")
        st.log("  ✓ 16 OSPF neighbor adjacencies formed (Full state)")
        ...
        if validation_failures:
            ...
            for dut in [dut1, dut2, dut3, dut4]:
                try:
                    st.generate_tech_support(dut=dut, name="ospf_scalability_ethernet_validation_failure")
