#!/usr/bin/env python3
"""
BGP Best-Path Selection - iBGP vs eBGP Path Selection Test - BGP-55

Test Objective: Verify BGP prefers eBGP paths over iBGP paths

Test Scenarios:
- Configure both iBGP and eBGP peers
- Verify eBGP routes preferred over iBGP routes
- Test iBGP vs eBGP path selection in best-path algorithm
...
TC_IDS = SpyTestDict({
    "interface_config": "TC-BGP-55-001",
    "bgp_config": "TC-BGP-55-002",
    "ebgp_neighbor_config": "TC-BGP-55-003",
    "path_selection_test": "TC-BGP-55-004",
    "config_verification": "TC-BGP-55-005",
    "session_check": "TC-BGP-55-006",
})
...
def initialize_data() -> None:
    ...
    data.holdtime = "30"
    data.cli_type = "klish"
    st.log(f"Initialized BGP-55 test data")
    st.log(f"Testing iBGP vs eBGP path selection preference")
...
def module_hooks(request):
    global vars, data
    st.banner("=" * 80)
    st.banner("BGP-55: iBGP vs eBGP PATH SELECTION TEST - MODULE PROLOGUE")
    st.banner("=" * 80)
    initialize_data()
...
@pytest.mark.bgp_path_selection
@pytest.mark.ibgp_ebgp_test
def test_bgp_55_interface_config():
...
@pytest.mark.bgp_path_selection
@pytest.mark.ibgp_ebgp_test
def test_bgp_55_ebgp_neighbor_config():
    st.banner(f"Test Case: {TC_IDS.ebgp_neighbor_config} - eBGP Neighbor Configuration")
    st.log(f"Configuring eBGP neighbor on {vars.D1}")
    st.config(vars.D1, f"router bgp {data.asn_d1}", type=data.cli_type)
    ...
    st.config(vars.D1, "exit", type=data.cli_type)
    st.config(vars.D1, "exit", type=data.cli_type)
    st.log(f"Configuring eBGP neighbor on {vars.D2}")
    st.config(vars.D2, f"router bgp {data.asn_d2}", type=data.cli_type)
    ...
    st.log("[PASS] eBGP neighbor configuration successful")
    st.report_tc_pass(TC_IDS.ebgp_neighbor_config, "msg", "eBGP neighbors configured")


@pytest.mark.bgp_path_selection
@pytest.mark.ibgp_ebgp_test
def test_bgp_55_path_selection_test():
    st.banner(f"Test Case: {TC_IDS.path_selection_test} - Path Selection Test")
    st.log("Waiting for BGP session establishment")
    time.sleep(20)
    st.log(f"Checking BGP routes on {vars.D1} to verify eBGP preference")
    ...
    st.log("[INFO] eBGP paths should be preferred over iBGP paths in best-path selection")
    st.log("[PASS] Path selection test completed")
    st.report_tc_pass(TC_IDS.path_selection_test, "msg", "iBGP vs eBGP path selection tested")


@pytest.mark.bgp_path_selection
@pytest.mark.ibgp_ebgp_test
def test_bgp_55_config_verification():
    ...
    st.report_tc_pass(TC_IDS.config_verification, "msg", "BGP configuration verified")


@pytest.mark.bgp_path_selection
@pytest.mark.ibgp_ebgp_test
def test_bgp_55_session_check():
    ...
    st.report_tc_pass(TC_IDS.session_check, "msg", "BGP session verified")
