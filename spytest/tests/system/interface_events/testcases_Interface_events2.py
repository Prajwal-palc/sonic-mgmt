"""
INTERFACE EVENTS - Admin/Link State Changes

How to run:
  cd Siddu/sonic-mgmt/spytest
  source spytest_venv/bin/activate
  ./bin/spytest --tryssh 1 --testbed ./testbeds/testbed_2vs.yaml \
  system/interface_events/testcases_Interface_events1.py \
  --logs-path ./logs/test_interface_events_$(date +%F_%H%M%S) \
  --log-level debug --skip-init-config --ifname-type native

Description:
  Validates CLI/syslog for interface administrative and link state changes.
  The test performs multiple shutdown/no-shutdown cycles on interfaces from
  the testbed to verify that state transitions are accurately reflected in
  CLI outputs. Each iteration validates admin state and operational state
  changes through show interface status command. Syslog is checked using
  'sudo tail -f /var/log/syslog' command outside of sonic-cli.
...
Pre-requisites:
  ...
  - Test ID: TC_INTF_EVENTS_001 (Test Plan 1.1.10)
"""
...
class TestInterfaceEvents:
    """Testcases covering interface admin/link state changes validation."""

    @classmethod
    def setup_class(cls):
        """Initialize test environment and collect topology information."""
        st.log("=" * 80)
        st.log("Setting up TestInterfaceEvents class")
        st.log("=" * 80)
    ...
    def teardown_class(cls):
        ...
        st.log("=" * 80)
        st.log("Tearing down TestInterfaceEvents class")
        st.log("=" * 80)
    ...
    @pytest.mark.inventory(feature="Regression", testcases=["TC_INTF_EVENTS_001"])
    def test_interface_admin_state_changes(self):
        """
        TC_INTF_EVENTS_001: Validate CLI/syslog for admin/link state changes.
        ...
        """
        st.log("=" * 80)
        st.log("TEST: Validate Interface Admin/Link State Changes")
        st.log("=" * 80)
        ...
        st.log("")
        st.log("=" * 80)
        st.log("FINAL VALIDATION - Test Summary")
        st.log("=" * 80)
        ...
        if all_passed:
            ...
            st.log(f"All {self.iterations} iterations completed successfully")
            st.log("Interface state transitions validated correctly")
            st.log("=" * 80)
