"""
IPv6 BGP NEGATIVE TEST - WRONG PASSWORD AUTHENTICATION
Author: Prajwal

How to run:
  ./bin/spytest  --tryssh 1  \
  --testbed ./testbeds/testbed_vs_2d.yaml  \
  tests/routing/bgp/test_ipv6_bgp_negative_password.py \
  --logs-path ./logs/test_ipv6_bgp_negative_password_$(date +%F_%H%M%S) \
...
def ipv6_bgp_negative_password_module_hooks(request):
    """
    Module-level fixture for IPv6 BGP negative password test setup and teardown.
    """
    global vars
    ...
    st.banner("MODULE SETUP: IPv6 BGP Negative Test (Wrong Password)")
...
def ipv6_bgp_negative_password_func_hooks(request):
    """
...
class TestIpv6BgpNegativePassword:
    """
    Test class for IPv6 BGP negative test - wrong password authentication.
    """

    def test_ipv6_bgp_password_mismatch(self):
        """
        ...
        """
        st.banner("NEGATIVE TEST: IPv6 BGP - Password Mismatch")
        ...
        if not configure_bgp_neighbor_with_password(
            data.dut1, data.bgp_asn, data.dut2_ipv6, data.bgp_asn,
            data.correct_password, data.af_ipv6, data.cli_type
        ):
            st.report_fail("msg", f"Failed to configure BGP neighbor on {data.dut1}")

        if not configure_bgp_neighbor_with_password(
            data.dut2, data.bgp_asn, data.dut1_ipv6, data.bgp_asn,
            data.correct_password, data.af_ipv6, data.cli_type
        ):
        ...
        if not reconfigure_bgp_neighbor_password(
            data.dut1, data.bgp_asn, data.dut2_ipv6, data.bgp_asn,
            data.wrong_password, data.af_ipv6, data.cli_type
        ):
        ...
        if not verify_bgp_neighbor_connect_state(data.dut1, data.dut2_ipv6, data.negative_test_wait, data.cli_type):
