"""
IPv4 BGP DAEMON RESTART - RECOVERY VERIFICATION
Author: Prajwal

How to run:
  ./bin/spytest  --tryssh 1  \
  --testbed ./testbeds/testbed_vs_2d.yaml  \
  tests/routing/bgp/test_ipv4_bgp_daemon_restart.py \
  --logs-path ./logs/test_ipv4_bgp_daemon_restart_$(date +%F_%H%M%S) \
  --log-level debug  --skip-init-config  --ifname-type native

Description:
  Test case to verify BGP session recovery after BGP daemon restart.
  This test validates that BGP sessions, routes, and connectivity are properly
  ...
  Test scenarios:
  1. Configure iBGP session between DUT1 and DUT2
  2. Advertise network from DUT1
  ...
  7. Verify end-to-end connectivity restored
...
data.advertised_network = "10.1.1.0/24"
data.advertised_network_prefix = "10.1.1.0"
...
def ipv4_bgp_daemon_restart_module_hooks(request):
    """
    Module-level fixture for IPv4 BGP daemon restart test setup and teardown.
    """
    global vars
    ...
    st.banner("MODULE SETUP: IPv4 BGP Daemon Restart Test")
    ...
    # Module teardown
    st.banner("MODULE TEARDOWN: Cleaning up IPv4 BGP daemon restart test configuration")
    cleanup_bgp_ipv4_config()


@pytest.fixture(scope="function", autouse=True)
def ipv4_bgp_daemon_restart_func_hooks(request):
    """
...
def advertise_bgp_network(dut, local_asn, network, cli_type="klish"):
    """
    ...
    try:
        ...
        st.config(dut, commands, type=cli_type)
        st.log(f"Successfully advertised network in BGP on {dut}")
        return True
    except Exception as e:
...
def verify_bgp_routes_from_neighbor(dut, neighbor_ip, expected_routes=None, min_routes=1,
    ...
    try:
        ...
        # Look for lines like: "*>i 192.168.10.0/24    10.1.1.1    0    100      0 i"
        # IPv4 pattern: matches routes like "192.168.10.0/24" or "10.0.0.0/8"
        route_pattern = r'\*?\s*[>i]\s*([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/\d+)'
...
def restart_bgp_daemon(dut, cli_type='click'):
    ...
    try:
        ...
        command = "sudo systemctl restart bgp"
        output = st.config(dut, command, type=cli_type, skip_error_check=True)
        ...
        st.wait(10, "Waiting for BGP daemon to restart")
...
class TestIpv4BgpDaemonRestart:
    """
    Test class for IPv4 BGP daemon restart recovery test.
    """

    def test_ipv4_bgp_daemon_restart_recovery(self):
        """
        Test: Verify BGP session recovery after BGP daemon restart.
        ...
        """
        st.banner("TEST: IPv4 BGP Daemon Restart Recovery")
        ...
        if not advertise_bgp_network(data.dut1, data.bgp_asn, data.advertised_network, data.cli_type):
        ...
        st.log(f"Step 10: Verifying DUT2 learns advertised route {data.advertised_network_prefix} from neighbor {data.dut1_ipv4}")

        if not verify_bgp_routes_from_neighbor(data.dut2, data.dut1_ipv4,
                                                expected_routes=[data.advertised_network],
                                                min_routes=1, family='ipv4', cli_type=data.cli_type):
            st.report_fail("msg", f"DUT2 did not learn advertised route {data.advertised_network} from neighbor {data.dut1_ipv4}")
        ...
        if not verify_bgp_neighbor_state(data.dut1, data.dut2_ipv4, 'Established', data.af_ipv4, data.recovery_wait, data.cli_type):
            st.report_fail("msg", f"BGP session did NOT recover on {data.dut1} after daemon restart")

        if not verify_bgp_neighbor_state(data.dut2, data.dut1_ipv4, 'Established', data.af_ipv4, data.recovery_wait, data.cli_type):
        ...
        st.log(f"Step 13: Verifying DUT2 re-learns advertised route {data.advertised_network} from neighbor {data.dut1_ipv4} after restart")

        if not verify_bgp_routes_from_neighbor(data.dut2, data.dut1_ipv4,
                                                expected_routes=[data.advertised_network],
                                                min_routes=1, family='ipv4', cli_type=data.cli_type):
            st.report_fail("msg", f"DUT2 did not re-learn advertised route {data.advertised_network} from neighbor {data.dut1_ipv4} after daemon restart")

        st.log("POST-RESTART: Routes re-learned successfully")

        # Step 14: Verify connectivity is restored
        st.log("Step 14: Verifying IPv4 connectivity is restored after daemon restart")
