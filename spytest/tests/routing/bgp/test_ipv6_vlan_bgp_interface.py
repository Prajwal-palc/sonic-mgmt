...
class TestIpv6VlanBgpInterface:
    ...
    def test_ipv6_vlan_bgp_interface_config_verify(self):
        ...
        # Step 8: Wait and verify BGP session establishment
        st.log("Step 8: Verifying BGP session establishment")
        ...
        if not (result1 and result2):
            st.report_fail("msg", "BGP session failed to establish on one or both DUTs over VLAN")

        # Step 9: Verify detailed BGP neighbor information using sonic-cli (Klish)
        st.log("Step 9: Verifying detailed BGP neighbor information using sonic-cli")
