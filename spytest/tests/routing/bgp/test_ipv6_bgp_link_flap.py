"""
IPv6 BGP LINK FLAP TEST - INTERFACE DOWN UNEXPECTEDLY
Author: Prajwal

How to run:
  ./bin/spytest  --tryssh 1  \
  --testbed ./testbeds/testbed_vs_2d.yaml  \
  tests/routing/bgp/test_ipv6_bgp_link_flap.py \
  --logs-path ./logs/test_ipv6_bgp_link_flap_$(date +%F_%H%M%S) \
...
Pre-requisites:
  ...
  - Network advertised: 2001:db8:10::/64 from DUT1
...
data.advertised_network = "2001:db8:10::/64"
data.advertised_network_prefix = "2001:db8:10::"
...
data.advertised_network_ip = "2001:db8:10::4"
# BGP config
data.bgp_asn = "65001"
data.af_ipv6 = "ipv6"
data.cli_type = "klish"
...
def ipv6_bgp_link_flap_module_hooks(request):
    """
    Module-level fixture for IPv6 BGP link flap test setup and teardown.
    """
    global vars
    ...
    st.banner("MODULE SETUP: IPv6 BGP Link Flap Test")
...
def ipv6_bgp_link_flap_func_hooks(request):
    """
...
def configure_bgp_neighbor(dut, local_asn, neighbor_ip, remote_asn, family="ipv6", cli_type="klish"):
    """
    Configure BGP neighbor with IPv6 address family.
    """
...
def advertise_network(dut, local_asn, network, family="ipv6", cli_type="klish"):
    """
...
def verify_route_in_routing_table(dut, network, timeout=90, cli_type='klish'):
    ...
    Uses st.config to get raw output since template parsing may fail for IPv6.
    """
...
class TestIpv6BgpLinkFlap:
    """
    Test class for IPv6 BGP link flap test (interface down unexpectedly).
    """

    def test_ipv6_bgp_link_flap_storm(self):
        """
        ...
        3. Configure IPv6 addresses on physical interfaces
        ...
        """
        st.banner("TEST: IPv6 BGP Link Flap Storm (Interface Down Unexpectedly)")
        ...
        if not configure_bgp_neighbor(data.dut1, data.bgp_asn, data.dut2_ipv6, data.bgp_asn, data.af_ipv6, data.cli_type):
            st.report_fail("msg", f"Failed to configure BGP neighbor on {data.dut1}")

        if not configure_bgp_neighbor(data.dut2, data.bgp_asn, data.dut1_ipv6, data.bgp_asn, data.af_ipv6, data.cli_type):
        ...
        if not configure_ipv6_interface(data.dut1, data.advertised_network_interface,
        ...
        if not advertise_network(data.dut1, data.bgp_asn, data.advertised_network, data.af_ipv6, data.cli_type):
        ...
        # Step 10: Verify route is installed on DUT2
        st.log("Step 10: Verifying route is installed on DUT2")
        ...
        # Step 11: Simulate link flap storm on DUT1's interface
        st.log("Step 11: Simulating link flap storm on DUT1's interface")
        ...
        # Step 12: Verify BGP session recovers after flaps
        st.log("Step 12: Verifying BGP session recovers after link flaps")
        ...
        # Step 13: Verify route is re-installed after recovery
        st.log("Step 13: Verifying route is re-installed on DUT2 after recovery")
