"""
IPv4 BGP ROUTE REFLECTOR CONFIGURATION AND VERIFICATION
Author: Prajwal
...
Description:
  End-to-end validation of IPv4 BGP Route Reflector configuration over physical interfaces.
...
data = SpyTestDict()
# DUT1 - DUT2 link
data.dut1_ipv4 = "10.1.12.2"
data.dut2_ipv4_link1 = "10.1.12.4"
# DUT2 - DUT3 link
data.dut2_ipv4_link2 = "10.1.14.2"
data.dut3_ipv4 = "10.1.14.3"
data.ipv4_mask = "24"
data.bgp_asn = "65001"
data.af_ipv4 = "ipv4"
data.cli_type = "klish"
...
def ipv4_bgp_rr_module_hooks(request):
    """
    Module-level fixture for IPv4 BGP Route Reflector test setup and teardown.
    ...
    st.banner("MODULE SETUP: IPv4 BGP Route Reflector Test")
    ...
    # Module teardown
    st.banner("MODULE TEARDOWN: Cleaning up IPv4 BGP Route Reflector configuration")
    cleanup_bgp_ipv4_config()


@pytest.fixture(scope="function", autouse=True)
def ipv4_bgp_rr_func_hooks(request):
    """
...
def verify_ipv4_interface(dut, interface, ipv4_addr, mask):
    """
    Verify IPv4 address configuration on an interface using direct Klish commands.

    Args:
        dut: Device under test
        interface: Interface name
        ipv4_addr: Expected IPv4 address
...
def configure_bgp_neighbor_with_routes(dut, local_asn, neighbor_ip, remote_asn, network_prefix,
                                        redistribute=True, family="ipv4", cli_type="klish"):
    """
    Configure BGP neighbor with IPv4 address family, network advertisement, and redistribute connected.
    ...
    Args:
        ...
        network_prefix: Network to advertise (e.g., "10.1.12.2/24")
...
def configure_bgp_route_reflector_client(dut, local_asn, neighbor_ip, remote_asn,
                                          network_prefixes=None, family="ipv4", cli_type="klish"):
    """
    ...
    Args:
        ...
        local_asn: Local AS number
        neighbor_ip: Neighbor IPv4 address (can be list for multiple neighbors)
        remote_asn: Remote AS number
...
def verify_route_reflector_client(dut, neighbor_ip, cli_type='klish'):
    ...
    try:
        ...
        command = f"show bgp ipv4 unicast neighbors {neighbor_ip}"
...
def verify_bgp_routes_from_neighbor(dut, neighbor_ip, expected_routes=None, min_routes=1,
    ...
    """
    Verify routes received from a BGP neighbor using 'show bgp ipv4 unicast neighbors X routes'.
    ...
    try:
        command = f"show bgp ipv4 unicast neighbors {neighbor_ip} routes"
        output = st.config(dut, command, type=cli_type, skip_error_check=True)
...
def ping_ipv4(dut, destination_ipv4, count=5, cli_type='click'):
    ...
    Args:
        dut: Device under test
        destination_ipv4: Destination IPv4 address
...
def cleanup_bgp_ipv4_config():
    ...
    dut_configs = [
        (data.dut1, data.dut1_dut2_port, data.dut1_ipv4),
        (data.dut2, data.dut2_dut1_port, data.dut2_ipv4_link1),
        (data.dut2, data.dut2_dut3_port, data.dut2_ipv4_link2),
        (data.dut3, data.dut3_dut2_port, data.dut3_ipv4)
    ]

    for dut, interface, ipv4_addr in dut_configs:
        try:
...
class TestIpv6BgpRouteReflector:
    """
    Test class for IPv4 BGP Route Reflector configuration and verification.
    """

    def test_ipv4_bgp_route_reflector_config_verify(self):
        """
        Test IPv4 BGP Route Reflector configuration and route propagation.

        Topology:
        DUT1 (RR Client) <---> DUT2 (RR Server) <---> DUT3 (RR Client)

        Steps:
        1. Check and remove existing IPv4 addresses from interfaces
        2. Configure interfaces with IPv4 addresses
        ...
        """
        st.banner("TEST: IPv4 BGP Route Reflector Configuration and Verification")
        ...
        remove_ipv4_address(data.dut2, data.dut2_dut3_port, data.cli_type)
        remove_ipv4_address(data.dut3, data.dut3_dut2_port, data.cli_type)
        ...
        st.log("Step 3: Configuring IPv4 addresses on all interfaces")
        ...
        if not configure_ipv4_interface(data.dut2, data.dut2_dut1_port, data.dut2_ipv4_link1, data.ipv4_mask, data.cli_type):
            st.report_fail("msg", f"Failed to configure IPv4 on {data.dut2} link1")

        if not configure_ipv4_interface(data.dut2, data.dut2_dut3_port, data.dut2_ipv4_link2, data.ipv4_mask, data.cli_type):
            st.report_fail("msg", f"Failed to configure IPv4 on {data.dut2} link2")

        if not configure_ipv4_interface(data.dut3, data.dut3_dut2_port, data.dut3_ipv4, data.ipv4_mask, data.cli_type):
            st.report_fail("msg", f"Failed to configure IPv4 on {data.dut3}")

        # Step 4: Verify IPv4 configuration
        st.log("Step 4: Verifying IPv4 configuration on all interfaces")
        ...
        if not verify_ipv4_interface(data.dut2, data.dut2_dut1_port, data.dut2_ipv4_link1, data.ipv4_mask):
            st.report_fail("msg", f"IPv4 verification failed on {data.dut2} link1")

        if not verify_ipv4_interface(data.dut2, data.dut2_dut3_port, data.dut2_ipv4_link2, data.ipv4_mask):
            st.report_fail("msg", f"IPv4 verification failed on {data.dut2} link2")

        if not verify_ipv4_interface(data.dut3, data.dut3_dut2_port, data.dut3_ipv4, data.ipv4_mask):
            st.report_fail("msg", f"IPv4 verification failed on {data.dut3}")

        # Step 5: Test IPv4 connectivity
        st.log("Step 5: Testing IPv4 connectivity between neighbors")

        if not ping_ipv4(data.dut1, data.dut2_ipv4_link1, data.ping_count):
            st.report_fail("msg", f"Ping from {data.dut1} to {data.dut2} failed")

        if not ping_ipv4(data.dut3, data.dut2_ipv4_link2, data.ping_count):
        ...
        if not configure_bgp_neighbor_with_routes(
            data.dut1, data.bgp_asn, data.dut2_ipv4_link1, data.bgp_asn,
            f"{data.dut1_ipv4}/{data.ipv4_mask}", redistribute=False,
            family=data.af_ipv4, cli_type=data.cli_type
        ):
        ...
        if not configure_bgp_neighbor_with_routes(
            data.dut3, data.bgp_asn, data.dut2_ipv4_link2, data.bgp_asn,
            f"{data.dut3_ipv4}/{data.ipv4_mask}", redistribute=False,
            family=data.af_ipv4, cli_type=data.cli_type
        ):
        ...
        if not configure_bgp_route_reflector_client(
            ...
            [data.dut1_ipv4, data.dut3_ipv4],  # Both neighbors are RR clients
            data.bgp_asn,
            network_prefixes=[f"{data.dut2_ipv4_link1}/{data.ipv4_mask}",
                             f"{data.dut2_ipv4_link2}/{data.ipv4_mask}"],
            family=data.af_ipv4, cli_type=data.cli_type
        ):
        ...
        result1 = verify_bgp_neighbor_state(
            data.dut1, data.dut2_ipv4_link1, 'Established', data.af_ipv4, data.bgp_wait, data.cli_type
        )
        ...
        result3 = verify_bgp_neighbor_state(
            data.dut2, data.dut3_ipv4, 'Established', data.af_ipv4, data.bgp_wait, data.cli_type
        )

        # Verify DUT3 <-> DUT2 session
        result4 = verify_bgp_neighbor_state(
            data.dut3, data.dut2_ipv4_link2, 'Established', data.af_ipv4, data.bgp_wait, data.cli_type
        )
        ...
        rr_result1 = verify_route_reflector_client(data.dut2, data.dut1_ipv4, data.cli_type)
        if not rr_result1:
            st.report_fail("msg", f"Route-Reflector Client verification failed for {data.dut1_ipv4} on {data.dut2}")

        # Verify DUT3 is configured as RR client on DUT2
        rr_result2 = verify_route_reflector_client(data.dut2, data.dut3_ipv4, data.cli_type)
        if not rr_result2:
            st.report_fail("msg", f"Route-Reflector Client verification failed for {data.dut3_ipv4} on {data.dut2}")
        ...
        if not verify_bgp_routes_from_neighbor(
            data.dut3, data.dut2_ipv4_link2,
            expected_routes=[f"{data.dut1_ipv4}"],
            min_routes=1, family=data.af_ipv4, cli_type=data.cli_type
        ):
        ...
        if not verify_bgp_routes_from_neighbor(
            data.dut1, data.dut2_ipv4_link1,
            expected_routes=[f"{data.dut3_ipv4}"],
            min_routes=1, family=data.af_ipv4, cli_type=data.cli_type
        ):
            st.log("Route reflection from DUT3 to DUT1 verification needs more time")

        st.log("IPv4 BGP Route Reflector configuration and verification test PASSED")
        st.report_pass("test_case_passed")

    def test_ipv4_bgp_route_reflector_save_reboot(self):
        """
        Test IPv4 BGP Route Reflector persistence after save and reboot.
        ...
        """
        st.banner("TEST: IPv4 BGP Route Reflector Persistence After Reboot")
        ...
        verify_bgp_routes_from_neighbor(
            data.dut3, data.dut2_ipv4_link2, min_routes=1, family=data.af_ipv4, cli_type=data.cli_type
        )

        verify_bgp_routes_from_neighbor(
            data.dut1, data.dut2_ipv4_link1, min_routes=1, family=data.af_ipv4, cli_type=data.cli_type
        )

        st.log("IPv4 BGP Route Reflector persistence after reboot test PASSED")
        st.report_pass("test_case_passed")
