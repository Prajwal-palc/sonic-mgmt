"""
IPv6 PHYSICAL INTERFACE WITH BGP - Show commands on DUT1 only
Author: Athira
...
How to run:
  ...
  tests/routing/bgp/test_ipv6_bgp_interface_physical.py \
  --logs-path ./logs/test_ipv6_bgp_interface_physical_$(date +%F_%H%M%S) \
...
Description:
  ...
  BGP sessions and connectivity persist after reboot.

  NOTE: All show commands are executed on DUT1 only for validation purposes.
...
def ipv6_bgp_module_hooks(request):
    ...
    st.banner("MODULE SETUP: IPv6 BGP Interface Test (Physical)")
...
class TestIpv6BgpInterfacePhysical:
    """
    Test class for IPv6 BGP over physical interface validation.
    NOTE: All show commands executed on DUT1 only.
    """
    ...
    def test_ipv6_bgp_interface_config_verify(self):
        ...
        3. Verify IPv6 address configuration (DUT1 only)
        4. Test IPv6 connectivity via ping (from DUT1 only)
        5. Configure BGP router and neighbors on both DUTs
        6. Verify BGP session establishment (DUT1 only)
        7. Verify BGP neighbor details (DUT1 only)

        Expected Result:
        - All configuration steps succeed
        - IPv6 connectivity verified via ping from DUT1
        - BGP session established (verified on DUT1)

        NOTE: Show commands executed on DUT1 only
        """
        st.banner("TEST: IPv6 BGP Interface Configuration and Verification (Physical - DUT1 Show Only)")
        ...
        # Step 3: Verify IPv6 address configuration (DUT1 only)
        st.log("Step 3: Verifying IPv6 address configuration on DUT1 only")
        ...
        if not result1:
            st.report_fail("msg", "Failed to verify IPv6 address on DUT1")

        # Step 4: Test IPv6 connectivity via ping (from DUT1 only)
        st.log("Step 4: Testing IPv6 connectivity via ping from DUT1 only")

        # Ping from DUT1 to DUT2
        result1 = verify_ipv6_ping(data.dut1, data.dut2_ipv6, data.ping_count)

        if not result1:
            st.report_fail("msg", "IPv6 ping failed from DUT1 to DUT2")

        # Step 5: Configure BGP router and neighbors on both DUTs
        st.log("Step 5: Configuring BGP router and neighbors on both DUTs")
        ...
        # Configure BGP on DUT2 (using alternative CLI hierarchy)
        ...
        # Step 6: Wait and verify BGP session establishment (DUT1 only)
        st.log("Step 6: Verifying BGP session establishment on DUT1 only")
        ...
        # Step 7: Verify detailed BGP neighbor information using sonic-cli (DUT1 only)
        st.log("Step 7: Verifying detailed BGP neighbor information using sonic-cli on DUT1 only")
    ...
    def test_ipv6_bgp_save_reboot(self):
        ...
        Test Steps:
        1. Verify BGP session is established (pre-check on DUT1 only)
        2. Verify IPv6 connectivity via ping (from DUT1 only)
        3. Check BGP routes (DUT1 only)
        4. Save configuration on all DUTs
        5. Reboot all DUTs
        6. Verify BGP sessions after reboot (DUT1 only)
        7. Verify IPv6 connectivity after reboot (from DUT1 only)

        Expected Result:
        - Configuration persists after reboot
        - BGP session re-establishes automatically (verified on DUT1)
        - IPv6 connectivity restored (verified from DUT1)

        NOTE: Show commands executed on DUT1 only
        """
        st.banner("TEST: IPv6 BGP Save and Reboot (Physical - DUT1 Show Only)")
        ...
        st.log("Step 1: Pre-reboot verification - checking BGP session on DUT1 only")
        ...
        # Step 2: Verify IPv6 connectivity via ping (pre-reboot from DUT1 only)
        st.log("Step 2: Pre-reboot verification - testing IPv6 connectivity from DUT1 only")

        result1 = verify_ipv6_ping(data.dut1, data.dut2_ipv6, data.ping_count)

        if not result1:
            st.report_fail("msg", "IPv6 ping failed from DUT1 before save/reboot")

        # Step 3: Check BGP routes using sonic-cli (DUT1 only)
        st.log("Step 3: Checking BGP IPv6 routes using sonic-cli on DUT1 only")
        ...
        st.log("Step 6: Post-reboot verification - checking BGP session on DUT1 only")
        ...
        # Step 7: Verify IPv6 connectivity after reboot (from DUT1 only)
        st.log("Step 7: Post-reboot verification - testing IPv6 connectivity from DUT1 only")

        result1 = verify_ipv6_ping(data.dut1, data.dut2_ipv6, data.ping_count)

        if not result1:
            st.report_fail("msg", "IPv6 ping failed from DUT1 after reboot")

        # Verify detailed BGP information post-reboot using sonic-cli (DUT1 only)
        st.log("Verifying detailed BGP neighbor information after reboot using sonic-cli on DUT1 only")
