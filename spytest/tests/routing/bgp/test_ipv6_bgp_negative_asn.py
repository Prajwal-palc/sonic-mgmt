"""
IPv6 BGP NEGATIVE TEST - WRONG ASN CONFIGURATION
Author: Prajwal

How to run:
  ./bin/spytest  --tryssh 1  \
  --testbed ./testbeds/testbed_vs_2d.yaml  \
  tests/routing/bgp/test_ipv6_bgp_negative_asn.py \
  --logs-path ./logs/test_ipv6_bgp_negative_asn_$(date +%F_%H%M%S) \
...
def ipv6_bgp_negative_asn_module_hooks(request):
    """
    Module-level fixture for IPv6 BGP negative ASN test setup and teardown.
    """
    global vars

    vars = st.ensure_min_topology("D1D2:1")

    st.banner("MODULE SETUP: IPv6 BGP Negative ASN Test")
    ...
    st.banner("MODULE TEARDOWN: Cleaning up IPv6 BGP negative ASN test configuration")
    cleanup_bgp_ipv6_config()


@pytest.fixture(scope="function", autouse=True)
def ipv6_bgp_negative_asn_func_hooks(request):
    """
...
class TestIpv6BgpNegativeAsn:
    """
    Test class for IPv6 BGP negative test with wrong ASN.
    """

    def test_ipv6_bgp_wrong_asn_no_establish(self):
        """
        ...
        """
        st.banner("NEGATIVE TEST: IPv6 BGP Wrong ASN - Session Should NOT Establish")
        ...
        st.log("Step 3: Configuring IPv6 addresses on both DUTs")
        ...
        st.log("Step 4: Verifying IPv6 configuration on both DUTs")
        ...
        st.log("Step 5: Testing IPv6 connectivity before BGP configuration")
        ...
        if not configure_bgp_neighbor(
            data.dut1, data.dut1_bgp_asn, data.dut2_ipv6, data.wrong_asn, data.af_ipv6, data.cli_type
        ):
        ...
        if not configure_bgp_neighbor(
            data.dut2, data.dut2_bgp_asn, data.dut1_ipv6, data.dut1_bgp_asn, data.af_ipv6, data.cli_type
        ):
        ...
        result1 = verify_bgp_neighbor_not_established(
            data.dut1, data.dut2_ipv6, data.bgp_wait, data.cli_type
        )
        ...
        result2 = verify_bgp_neighbor_not_established(
            data.dut2, data.dut1_ipv6, data.bgp_wait, data.cli_type
        )
    ...
    def test_ipv6_bgp_correct_asn_after_fix(self):
        """
        ...
        """
        st.banner("POSITIVE TEST: IPv6 BGP Correct ASN - Session Should Establish After Fix")
        ...
        try:
            ...
            commands.append(f"no neighbor {data.dut2_ipv6}")
            commands.append("exit")
        ...
        if not configure_bgp_neighbor(
            data.dut1, data.dut1_bgp_asn, data.dut2_ipv6, data.dut2_bgp_asn, data.af_ipv6, data.cli_type
        ):
