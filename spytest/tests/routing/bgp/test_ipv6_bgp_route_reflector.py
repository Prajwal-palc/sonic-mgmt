...
def ipv6_bgp_rr_module_hooks(request):
    ...
    # Clean up any existing BGP and IPv6 configuration before tests
    st.banner("MODULE SETUP: Cleaning up existing BGP and IPv6 configuration")
...
def cleanup_existing_config():
    """
    Unconditionally remove existing BGP and IPv6 configuration from all DUTs.
    Does not check if configuration exists - just removes it directly.
    """
    st.log("Removing existing BGP and IPv6 configuration from all DUTs")

    dut_configs = [
        (data.dut1, data.dut1_dut2_port),
        (data.dut2, data.dut2_dut1_port),
        (data.dut2, data.dut2_dut3_port),
        (data.dut3, data.dut3_dut2_port)
    ]

    for dut, interface in dut_configs:
        try:
            ...
            st.log(f"Cleanup completed on {dut} interface {interface}")

        except Exception as e:
            st.log(f"Cleanup on {dut} encountered: {str(e)} - continuing")

    st.log("Initial cleanup completed on all DUTs")
...
class TestIpv6BgpRouteReflector:
    ...
    def test_ipv6_bgp_route_reflector_config_verify(self):
        ...
        Steps:
        1. Configure interface MTU and speed
        ...
        Note: BGP and IPv6 cleanup is done unconditionally in module setup.
        """
        st.banner("TEST: IPv6 BGP Route Reflector Configuration and Verification")

        # Step 1: Configure interface MTU and speed
        st.log("Step 1: Configuring interface MTU and speed on all DUTs")
        ...
        st.log("Step 2: Configuring IPv6 addresses on all interfaces")
        ...
        st.log("Step 3: Verifying IPv6 configuration on all interfaces")
        ...
        st.log("Step 4: Testing IPv6 connectivity between neighbors")
        ...
        st.log("Step 5: Configuring BGP routers with router IDs on all DUTs")
        ...
        # Step 6: Configure BGP neighbors on DUT1 (RR Client)
        st.log("Step 6: Configuring BGP neighbor on DUT1 (RR Client)")
        ...
        # Step 7: Configure BGP neighbors on DUT3 (RR Client)
        st.log("Step 7: Configuring BGP neighbor on DUT3 (RR Client)")
        ...
        # Step 8: Configure DUT2 as Route Reflector Server
        st.log("Step 8: Configuring DUT2 as Route Reflector Server")
