"""
IPv4 BGP LOOPBACK NEGATIVE TEST - WRONG UPDATE-SOURCE
Author: Prajwal

How to run:
  ./bin/spytest  --tryssh 1  \
  --testbed ./testbeds/testbed_vs_2d.yaml  \
  tests/routing/bgp/test_ipv4_bgp_loopback_negative_updatesource.py \
  --logs-path ./logs/test_ipv4_bgp_loopback_negative_updatesource_$(date +%F_%H%M%S) \
...
Pre-requisites:
  ...
  - Topology Diagram:
        ...
        # +-------------------------+                       +-------------------------+
        # |      DUT1               |                       |      DUT2               |
        # | Loopback0: 10.0.0.1  |                       | Loopback0: 10.0.0.2  |
...
data.dut1_loopback = "10.2.0.1"
data.dut2_loopback = "10.2.0.2"
...
def ipv4_bgp_loopback_negative_module_hooks(request):
    """
    Module-level fixture for IPv4 BGP Loopback negative test setup and teardown.
    """
    global vars
    ...
    st.banner("MODULE SETUP: IPv4 BGP Loopback Negative Test (Wrong Update-Source)")
    ...
    # Module teardown
    st.banner("MODULE TEARDOWN: Cleaning up IPv4 BGP Loopback configuration")
    cleanup_bgp_ipv4_loopback_config()


@pytest.fixture(scope="function", autouse=True)
def ipv4_bgp_loopback_negative_func_hooks(request):
    """
...
def configure_static_route(dut, destination, nexthop, cli_type="klish"):
    """
    Configure IPv4 static route for loopback reachability.
    """
...
def configure_bgp_neighbor_loopback_wrong_updatesource(dut, local_asn, neighbor_ip, remote_asn,
    ...
    Args:
        ...
        neighbor_ip: Neighbor IPv4 loopback address
...
def cleanup_bgp_ipv4_loopback_config():
    """
    Clean up BGP, IPv4, and Loopback configuration from all DUTs.
    """
    st.log("Cleaning up BGP, IPv4, and Loopback configuration")
    ...
    for dut in dut_list:
        try:
            ...
            if dut == data.dut1:
                commands = [f"no ip route {data.dut2_loopback}/{data.loopback_mask} {data.dut2_ipv4}"]
            else:
...
class TestIpv6BgpLoopbackNegativeUpdateSource:
    """
    Test class for IPv4 BGP Loopback negative test - wrong update-source.
    """

    def test_ipv4_bgp_loopback_wrong_updatesource(self):
        """
        Negative Test: Verify BGP session behavior with wrong update-source.

        Steps:
        1. Configure physical interfaces with IPv4 (underlay)
        ...
        """
        st.banner("NEGATIVE TEST: IPv4 BGP Loopback - Wrong Update-Source")
        ...
        # Step 3: Configure IPv4 addresses on physical interfaces (underlay)
        st.log("Step 3: Configuring IPv4 addresses on physical interfaces (underlay)")
        ...
        if not configure_bgp_neighbor_loopback_wrong_updatesource(
            ...
            data.wrong_loopback, data.ebgp_multihop, data.af_ipv4, data.cli_type
        ):
        ...
        if not configure_bgp_neighbor_loopback_correct_updatesource(
            ...
            data.loopback_interface, data.ebgp_multihop, data.af_ipv4, data.cli_type
        ):
        ...
        if not verify_bgp_neighbor_state(data.dut1, data.dut2_loopback, 'Established', data.af_ipv4, data.bgp_wait, data.cli_type):
            st.report_fail("msg", f"BGP session not established on {data.dut1} after fixing update-source")

        if not verify_bgp_neighbor_state(data.dut2, data.dut1_loopback, 'Established', data.af_ipv4, data.bgp_wait, data.cli_type):
