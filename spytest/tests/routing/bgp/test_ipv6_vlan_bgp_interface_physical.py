...
Description:
  End-to-end validation of IPv6 BGP peering over VLAN interfaces using Klish CLI.
  This test suite provisions VLAN 100, configures Ethernet32 as access port, creates
  VLAN interface (Vlan100) with IPv6 addresses, establishes iBGP sessions between
  two SONiC devices over the VLAN, validates BGP neighborship, verifies IPv6
...
Pre-requisites:
  ...
  - Router ID: 1.1.1.1 on both DUTs
...
def ipv6_vlan_bgp_module_hooks(request):
    """
    Module-level fixture for IPv6 VLAN BGP test setup and teardown.
    Sets up topology, configures VLANs, VLAN interfaces, IPv6 addresses, and BGP.
    """
    global vars
    ...
    st.banner("MODULE SETUP: IPv6 VLAN BGP Interface Test")
    ...
    # Module teardown
    st.banner("MODULE TEARDOWN: Cleaning up IPv6 VLAN BGP configuration")
    cleanup_vlan_bgp_config()


@pytest.fixture(scope="function", autouse=True)
def ipv6_vlan_bgp_func_hooks(request):
    """
...
def create_vlan_dut2_style(dut, vlan_id, cli_type="klish"):
    """
    Create VLAN on DUT2 using interface Vlan command (simpler style).
    This automatically creates the VLAN when entering the interface configuration.

    Args:
        dut: Device under test (DUT2)
        vlan_id: VLAN ID to create
        cli_type: CLI type (default: klish)
    ...
    Returns:
        bool: True if successful, False otherwise
    """
    st.log(f"Creating VLAN {vlan_id} on {dut} using DUT2 style (interface Vlan{vlan_id})")

    try:
        # Use interface Vlan command which auto-creates the VLAN
        commands = []
        commands.append(f"interface Vlan{vlan_id}")
        commands.append("exit")

        st.config(dut, commands, type=cli_type)
        st.log(f"Successfully created VLAN {vlan_id} on {dut} using DUT2 style")
        return True
    except Exception as e:
        st.error(f"Exception creating VLAN {vlan_id} on {dut}: {str(e)}")
        return False
...
def configure_bgp_neighbor_dut2_style(dut, local_asn, neighbor_ip, remote_asn, family="ipv6", cli_type="klish"):
    """
    Configure BGP neighbor using alternative CLI hierarchy (for DUT2).
    This uses neighbor submode configuration style:
    1. router bgp <asn>
    2. neighbor <ip>
    3. remote-as <asn>
    ...
    Returns:
        bool: True if successful, False otherwise
    """
    st.log(f"Configuring BGP neighbor {neighbor_ip} (DUT2 style) with AS {remote_asn} on {dut}")

    try:
        # Use alternative CLI hierarchy for DUT2
        commands = []
        commands.append(f"neighbor {neighbor_ip}")
        commands.append(f"remote-as {remote_asn}")
        commands.append(f"address-family ipv6 unicast")
        commands.append(f"activate")
        commands.append("exit")
        commands.append("exit")
        ...
        st.config(dut, commands, type=cli_type)
        st.log(f"Successfully configured BGP neighbor {neighbor_ip} on {dut} using DUT2 style")
        return True
    except Exception as e:
        st.error(f"Failed to configure BGP neighbor {neighbor_ip} on {dut}: {str(e)}")
        return False
...
def cleanup_vlan_bgp_config():
    ...
    """
    st.log("Cleaning up VLAN BGP and IPv6 configuration")
...
class TestIpv6VlanBgpInterface:
    """
    Test class for IPv6 BGP over VLAN interface validation.
    """

    @pytest.mark.test_ipv6_vlan_bgp_basic
    def test_ipv6_vlan_bgp_interface_config_verify(self):
        """
        TestCase: test_ipv6_vlan_bgp_interface_config_verify
        ...
        """
        st.banner("TEST: IPv6 VLAN BGP Interface Configuration and Verification")

        # Step 1: Create VLAN 100 on both DUTs
        st.log("Step 1: Creating VLAN 100 on both DUTs")

        # Create VLAN on DUT1 using standard style
        result1 = create_vlan(data.dut1, data.vlan_id, data.cli_type)

        # Create VLAN on DUT2 using DUT2 style (interface Vlan100)
        result2 = create_vlan_dut2_style(data.dut2, data.vlan_id, data.cli_type)

        if not (result1 and result2):
            st.report_fail("msg", "Failed to create VLAN on one or both DUTs")

        # Step 1.1: Verify VLAN creation using show Vlan on DUT1 only
        st.log("Step 1.1: Verifying VLAN creation on DUT1 using show Vlan")

        result1 = verify_vlan_config(data.dut1, data.vlan_id, data.cli_type)

        if not result1:
            st.report_fail("msg", "Failed to verify VLAN on DUT1")
        ...
        # Step 4: Verify IPv6 address configuration on DUT1 only
        st.log("Step 4: Verifying IPv6 address configuration on DUT1 VLAN interface")
        ...
        if not result1:
            st.report_fail("msg", "Failed to verify IPv6 address on DUT1 VLAN interface")
        ...
        # Step 6: Configure BGP router with router-id
        st.log("Step 6: Configuring BGP router with router-id on both DUTs")

        # Configure BGP on DUT1 with router-id 1.1.1.1
        ...
        # Configure BGP on DUT2 with router-id 2.2.2.2
        ...
        st.log("Step 7: Configuring BGP neighbors on both DUTs")

        # Configure BGP neighbor on DUT1 using standard style
        ...
        # Configure BGP neighbor on DUT2 using DUT2-specific style
        if not configure_bgp_neighbor_dut2_style(
        ...
        # Step 8: Wait and verify BGP session establishment (DUT1 only)
        st.log("Step 8: Verifying BGP session establishment on DUT1")
        ...
        if not result1:
            st.report_fail("msg", "BGP session failed to establish on DUT1 over VLAN")

        # Step 9: Verify detailed BGP neighbor information using sonic-cli (Klish) on DUT1 only
        st.log("Step 9: Verifying detailed BGP neighbor information on DUT1 using sonic-cli")
    ...
    @pytest.mark.test_ipv6_vlan_bgp_save_reboot
    def test_ipv6_vlan_bgp_save_reboot(self):
        """
        TestCase: test_ipv6_vlan_bgp_save_reboot
        ...
        """
        st.banner("TEST: IPv6 VLAN BGP Save and Reboot")

        # Ensure configuration is in place (run basic config first)
        st.log("Setting up IPv6 VLAN BGP configuration for save/reboot test")
        self.test_ipv6_vlan_bgp_interface_config_verify()

        # Step 1: Verify BGP session is established (pre-check on DUT1 only)
        st.log("Step 1: Pre-reboot verification - checking BGP session on DUT1")
        ...
        if not result1:
            st.report_fail("msg", "BGP session not established on DUT1 before save/reboot")
        ...
        # Step 3: Check BGP routes using sonic-cli (Klish) on DUT1 only
        st.log("Step 3: Checking BGP IPv6 routes on DUT1 using sonic-cli")
        ...
        # Step 6: Verify BGP session after reboot (DUT1 only)
        st.log("Step 6: Post-reboot verification - checking BGP session on DUT1")
        ...
        if not result1:
            st.report_fail("msg", "BGP session failed to re-establish on DUT1 after reboot")
        ...
        # Verify detailed BGP information post-reboot using sonic-cli (Klish) on DUT1 only
        st.log("Verifying detailed BGP neighbor information on DUT1 after reboot using sonic-cli")
