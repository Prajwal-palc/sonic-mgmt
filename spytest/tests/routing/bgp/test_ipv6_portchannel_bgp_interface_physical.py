...
Description:
  ...
  provisions IPv6 addresses on PortChannel interfaces, establishes iBGP sessions between
...
def portchannel_ipv6_bgp_module_hooks(request):
    """
    ...
    st.banner("MODULE SETUP: PortChannel IPv6 BGP Test")
    ...
    # Module teardown
    st.banner("MODULE TEARDOWN: Cleaning up PortChannel IPv6 BGP configuration")
    cleanup_portchannel_bgp_ipv6_config()


@pytest.fixture(scope="function", autouse=True)
def portchannel_ipv6_bgp_func_hooks(request):
    """
...
class TestPortchannelIpv6Bgp:
    ...
    @pytest.mark.test_portchannel_ipv6_bgp_basic
    def test_portchannel_ipv6_bgp_config_verify(self):
        """
        TestCase: test_portchannel_ipv6_bgp_config_verify
        ...
        # Step 5: Verify IPv6 address configuration on DUT1 only
        st.log("Step 5: Verifying IPv6 address configuration on DUT1 PortChannel interface")
        ...
        if not result1:
            st.report_fail("msg", "Failed to verify IPv6 address on DUT1 PortChannel interface")
        ...
        # Step 9: Wait and verify BGP session establishment (DUT1 only)
        st.log("Step 9: Verifying BGP session establishment on DUT1")
        ...
        # Step 10: Verify detailed BGP neighbor information on DUT1 using sonic-cli
        st.log("Step 10: Verifying detailed BGP neighbor information on DUT1 using sonic-cli")
        ...
        # Step 11: Verify BGP routes on DUT1 using sonic-cli
        st.log("Step 11: Checking BGP IPv6 routes on DUT1 using sonic-cli")
        ...
        # Step 15: Verify PortChannel and IPv6 configuration on DUT1 after reboot
        st.log("Step 15: Verifying PortChannel and IPv6 configuration on DUT1 after reboot")

        # Verify PortChannel status on DUT1 only
        result1 = verify_portchannel_status(data.dut1, data.portchannel)

        if not result1:
            st.report_fail("msg", "PortChannel not up on DUT1 after reboot")

        # Verify IPv6 configuration on DUT1 only
        result1 = verify_ipv6_on_portchannel(
            data.dut1, data.portchannel, data.dut1_ipv6, data.ipv6_mask
        )

        if not result1:
            st.report_fail("msg", "IPv6 configuration not present on DUT1 after reboot")

        # Step 16: Verify BGP session re-establishes after reboot (DUT1 only)
        st.log("Step 16: Verifying BGP session on DUT1 after reboot")
