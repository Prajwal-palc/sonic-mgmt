"""
STATIC ROUTE CONNECTIVITY (CLICK CLI)
Author: Athira
...
How to run:
  ...
  routing/static/test_static_route_click.py \
  --logs-path ./logs/test_static_route_click_$(date +%F_%H%M%S) \
  --log-level debug  --skip-init-config  --ifname-type native

Description:
  Validates test plan entry TC-IP-STATIC-001 by configuring IPv4 interfaces and
  a static route on DUT1 via the click CLI, then verifying reachability to the
  remote host hanging off DUT2. The flow provisions peer link addressing,
  programs the static route, sources ICMP from DUT1's Ethernet32, and performs
  cleanup to leave the fabric in the baseline state.
...
VAR_FILE_ENV = "STATIC_ROUTE_CLICK_VAR_FILE"
DEFAULT_VAR_FILE = (
    Path(__file__).resolve().parents[3]
    / "tests"
    / "routing"
    / "static"
    / "vars_static_route_click.yaml"
)
...
def _load_yaml_payload() -> SpyTestDict:
    ...
    if not candidate.is_file():
        raise FileNotFoundError(
            f"Static route click variable file not found: {candidate}"
        )
...
class TestStaticRouteClick:
    """SpyTest suite for TC-IP-STATIC-001 using the click CLI path."""
    ...
    def setup_class(cls) -> None:
        ...
        if not testcase:
            pytest.fail(
                "Missing 'static_route_via_dut2' definition in vars_static_route_click.yaml"
            )

        cli_type = (defaults.get("cli_type") or "click").strip().lower()
        if cli_type != "click":
            st.warn(
                f"Overriding cli_type={cli_type} with 'click' per static route testcase requirement"
            )
            cli_type = "click"
    ...
    def _resolve_alias(cls, alias: str):
        if not alias:
            pytest.fail("Topology alias missing in vars_static_route_click.yaml")
    ...
    def _build_interface_matrix(cls, testcase: SpyTestDict):
        ...
        if not entries:
            pytest.fail(
                "vars_static_route_click.yaml must define testcases.static_route_via_dut2.interfaces"
            )
        ...
        for raw_entry in entries:
            ...
            if not alias or not interface or ip_address is None or prefix is None:
                pytest.fail(
                    f"Incomplete interface entry in vars_static_route_click.yaml: {entry}"
                )
    ...
    def _build_static_route_block(cls, testcase: SpyTestDict, interface_matrix):
        ...
        if not route:
            pytest.fail(
                "Static route definition requires 'route' in vars_static_route_click.yaml"
            )
        try:
        ...
        if not next_hop:
            pytest.fail(
                "Static route definition requires 'next_hop' in vars_static_route_click.yaml"
            )
    ...
    def _build_ping_block(cls, testcase: SpyTestDict):
        ...
        if not target:
            pytest.fail(
                "Ping block requires 'target' in vars_static_route_click.yaml"
            )
    ...
    def test_static_route_reachability_click(self) -> None:
        """Validate IPv4 reachability after provisioning the static route via click CLI."""

        st.log(
            f"Executing {self.data.case_id}: verify ping to {self.data.ping.target} with click CLI static route"
        )
