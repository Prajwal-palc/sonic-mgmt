"""
OSPF BASIC CONFIGURATION - 4-NODE TOPOLOGY WITH PORTCHANNEL AND STATIC ROUTING
...
How to run:
  ...
  tests/system/iscli_OSPF/testcases_OSPF_4_iscli_Basic_4_node_PortChannel.py \
  --logs-path ./logs/testcases_OSPF_4_iscli_Basic_4_node_PortChannel_$(date +%F_%H%M%S) \
  --log-level debug --skip-init-config --ifname-type native

Description:
  This test validates OSPF configuration with PortChannel and static routing in a 4-node topology by:
  ...
  5. Restarting BGP docker and validating static routes in routing table
  6. Configuring OSPF on D2 and D4 (middle nodes)
  7. Verifying OSPF neighbor adjacency (Full state)
  8. Verifying DR/BDR election
  9. Validating ping connectivity from D1 to D3
...
class TestOSPFStaticRoutingPortChannel4Node:
    """Test cases for validating OSPF with PortChannel and static routing in 4-node topology via CLI (klish mode)."""
    ...
    def setup_class(cls) -> None:
        ...
        st.log("=" * 80)
        st.log("TEST SETUP: Initializing OSPF 4-Node PortChannel Static Routing Test Suite")
        st.log("=" * 80)
    ...
    def teardown_class(cls) -> None:
        """Cleanup test suite."""
        st.log("=" * 80)
        st.log("TEST TEARDOWN: Cleanup OSPF 4-Node PortChannel Static Routing Test Suite")
        st.log("=" * 80)
    ...
    def _verify_ipv4_in_running_config(config_output: str, expected_ipv4: Optional[str]) -> bool:
        ...
        actual_ipv4 = TestOSPFStaticRoutingPortChannel4Node._extract_ipv4_from_running_config(config_output)
    ...
    @pytest.mark.inventory(feature="Regression", testcases=["TC_OSPF_PORTCHANNEL_STATIC_4NODE_001"])
    def test_ospf_portchannel_static_routing_4node(self) -> None:
        """
        TC_OSPF_PORTCHANNEL_STATIC_4NODE_001: Validate OSPF with PortChannel and static routing in 4-node topology.
        ...
        """
        st.log("\n" + "=" * 80)
        st.log("TEST: OSPF with PortChannel and Static Routing - 4-Node Topology")
        st.log("=" * 80)
        ...
        #         st.report_fail("msg", f"Port {port} is not in PortChannel {self.data.pc110} on {dut1}")
        ...
        #         st.report_fail("msg", f"Port {port} is not in PortChannel {self.data.pc120} on {dut2}")
        ...
        #         st.report_fail("msg", f"Port {port} is not in PortChannel {self.data.pc130} on {dut4}")
        ...
        #         st.report_fail("msg", f"Port {port} is not in PortChannel {self.data.pc130} on {dut3}")

        st.log("PASS: PortChannels created successfully, all verified")
