...
class TestOSPFDRBDRElectionPortChannel:
    ...
    @staticmethod
    def _remove_ospf_priority(dut: str, portchannel_id: str) -> bool:
        """
        Remove OSPF priority from PortChannel interface.
        ...
        Returns:
            True if successful
        """
        st.log(f"Removing OSPF priority from PortChannel{portchannel_id} on {dut}")
        commands = [
            "configure terminal",
            f"interface PortChannel {portchannel_id}",
            "no ip ospf priority",
    ...
    def test_ospf_dr_bdr_election_with_priority_portchannel(self) -> None:
        ...
        # Remove OSPF priority from PortChannel interfaces
        self._remove_ospf_priority(dut2, portchannel_id)
        self._remove_ospf_priority(dut4, portchannel_id)
        st.log("OSPF priority removed from PortChannel interfaces on all DUTs")
        ...
        if validation_failures:
            ...
            try:
                st.generate_tech_support(dut=dut2, name="ospf_dr_bdr_election_pc_validation_failure")
            ...
            try:
                st.generate_tech_support(dut=dut4, name="ospf_dr_bdr_election_pc_validation_failure")
