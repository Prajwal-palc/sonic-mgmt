"""
OSPF SCALABILITY TESTING - 4-NODE TOPOLOGY WITH MULTIPLE VLAN LINKS
...
How to run:
  ...
  tests/system/iscli_OSPF/testcases_OSPF_14_iscli_4_node_OSPF_scalability_over_Vlan.py \
  --logs-path ./logs/testcases_OSPF_14_scalability_Vlan_$(date +%F_%H%M%S) \
  --log-level debug --skip-init-config --ifname-type native

Description:
  This test validates OSPF scalability in a 4-node topology with multiple VLAN
  links between devices. The test covers:

  1. Removing IP addresses from Ethernet interfaces before adding to VLANs
  2. Creating 12 VLANs (4 between each device pair)
  3. Adding Ethernet interfaces as members to respective VLANs
  4. Verifying VLAN creation and member addition
  5. Configuring IP addresses on all 12 VLAN interfaces
  6. Configuring OSPF Area 0 on all devices
  7. Verifying OSPF neighbor adjacency on all VLANs (Full state)
  ...
  Test Focus: OSPF scalability over VLAN interfaces
  - 4 devices with 4 parallel VLAN links between each pair
  - Total of 12 VLANs across the topology
  - Each VLAN has 1 Ethernet member
  ...
  Topology:
        D1 ======== (4 parallel VLANs) ======== D2
                                                 ||
                                                 ||
                          (4 parallel VLANs)     ||
                                                 ||
        D3 ======== (4 parallel VLANs) ======== D4

  VLANs:
    D1 ↔ D2: Vlan110,120,130,140 ↔ Vlan110,120,130,140
    D2 ↔ D4: Vlan150,160,170,180 ↔ Vlan150,160,170,180
    D4 ↔ D3: Vlan10,20,30,40 ↔ Vlan10,20,30,40

  VLAN Members (1 Ethernet port per VLAN as tagged member):
    D1 ↔ D2:
      Vlan110: Ethernet0 (D1) ↔ Ethernet0 (D2)
      Vlan120: Ethernet4 (D1) ↔ Ethernet4 (D2)
      Vlan130: Ethernet8 (D1) ↔ Ethernet8 (D2)
      Vlan140: Ethernet12 (D1) ↔ Ethernet12 (D2)
    D2 ↔ D4:
      Vlan150: Ethernet16 (D2) ↔ Ethernet16 (D4)
      Vlan160: Ethernet20 (D2) ↔ Ethernet20 (D4)
      Vlan170: Ethernet24 (D2) ↔ Ethernet24 (D4)
      Vlan180: Ethernet28 (D2) ↔ Ethernet28 (D4)
    D4 ↔ D3:
      Vlan10: Ethernet32 (D4) ↔ Ethernet32 (D3)
      Vlan20: Ethernet36 (D4) ↔ Ethernet36 (D3)
      Vlan30: Ethernet40 (D4) ↔ Ethernet40 (D3)
      Vlan40: Ethernet44 (D4) ↔ Ethernet44 (D3)

  IP Addressing:
    D1: Vlan110,120,130,140: 10.1.1.1/24, 10.1.2.1/24, 10.1.3.1/24, 10.1.4.1/24
    D2: Vlan110,120,130,140: 10.1.1.2/24, 10.1.2.2/24, 10.1.3.2/24, 10.1.4.2/24
        Vlan150,160,170,180: 20.1.1.1/24, 20.1.2.1/24, 20.1.3.1/24, 20.1.4.1/24
    D4: Vlan150,160,170,180: 20.1.1.2/24, 20.1.2.2/24, 20.1.3.2/24, 20.1.4.2/24
        Vlan10,20,30,40: 30.1.1.2/24, 30.1.2.2/24, 30.1.3.2/24, 30.1.4.2/24
    D3: Vlan10,20,30,40: 30.1.1.1/24, 30.1.2.1/24, 30.1.3.1/24, 30.1.4.1/24
  ...
  'show ip ospf neighbor detail', 'show ip ospf border-routers', and VLAN
...
class TestOSPFScalabilityOverVLAN:
    """Test cases for validating OSPF scalability over VLAN links in 4-node topology."""
    ...
    def setup_class(cls) -> None:
        ...
        st.log("=" * 80)
        st.log("TEST SETUP: Initializing OSPF Scalability Test Suite - VLAN Links")
        st.log("=" * 80)
    ...
    def teardown_class(cls) -> None:
        """Cleanup test suite."""
        st.log("=" * 80)
        st.log("TEST TEARDOWN: Cleanup OSPF Scalability Test Suite (VLAN)")
        st.log("=" * 80)
    ...
    @staticmethod
    def _add_member_to_vlan(dut: str, vlan_id: str, member_port: str) -> bool:
        """
        Add Ethernet interface as tagged member to VLAN.
        ...
        Returns:
            True if successful
        """
        st.log(f"Adding {member_port} to VLAN {vlan_id} on {dut}")
        commands = [
            "configure terminal",
    ...
    @staticmethod
    def _remove_member_from_vlan(dut: str, member_port: str, vlan_id: str) -> bool:
        """
        Remove Ethernet interface from VLAN.
        ...
        Returns:
            True if successful
        """
        st.log(f"Removing {member_port} from VLAN {vlan_id} on {dut}")
        commands = [
            "configure terminal",
            f"interface {member_port}",
    ...
    def _verify_ospf_interface_present(output: str, interface: str) -> bool:
        ...
        Args:
            ...
            interface: Interface name (e.g., "Vlan110")
    ...
    @pytest.mark.inventory(feature="Regression", testcases=["TC_OSPF_SCALABILITY_VLAN_001"])
    def test_ospf_scalability_over_vlan(self) -> None:
        """
        TC_OSPF_SCALABILITY_VLAN_001: Validate OSPF scalability over VLAN links.

        Test Procedure:
        1. Remove IP addresses from all Ethernet interfaces
        2. Create 12 VLANs (4 between each device pair)
        3. Add Ethernet members to VLANs (1 port per VLAN as tagged member)
        4. Configure IP addresses on all 12 VLAN interfaces
        5. Configure OSPF on all devices with all networks
        6. Verify OSPF neighbors form on all VLANs (Full state)
        ...
        - 12 VLANs created successfully
        - All Ethernet members added to respective VLANs
        - IP addresses configured on all 12 VLAN interfaces
        - OSPF neighbors form on all VLANs (12 adjacencies total)
        ...
        """
        st.log("\n" + "=" * 80)
        st.log("TEST: OSPF Scalability Over VLAN - 4-Node Topology with 12 VLANs")
        st.log("=" * 80)
        ...
        st.log("\n" + "-" * 80)
        st.log("STEP 1: Remove IP addresses from all Ethernet interfaces before adding to VLANs")
        st.log("-" * 80)
        ...
        # ===== STEP 2: Create VLANs =====
        st.log("\n" + "-" * 80)
        st.log("STEP 2: Create 12 VLANs (4 between each device pair)")
        st.log("-" * 80)
        ...
        st.log("All 12 VLANs created")
        time.sleep(WAIT_AFTER_VLAN_CONFIG)

        st.log("PASS: VLAN creation completed")

        # ===== STEP 3: Add Ethernet members to VLANs =====
        st.log("\n" + "-" * 80)
        st.log("STEP 3: Add Ethernet interfaces as tagged members to respective VLANs")
        st.log("-" * 80)

        # Add members for D1 ↔ D2 VLANs
        for i, (vlan_id, eth_port) in enumerate(zip(self.data.dut1_d2_vlans, self.data.dut1_d2_eth_ports)):
            self._add_member_to_vlan(dut1, vlan_id, eth_port)

        for i, (vlan_id, eth_port) in enumerate(zip(self.data.dut2_d1_vlans, self.data.dut2_d1_eth_ports)):
            self._add_member_to_vlan(dut2, vlan_id, eth_port)

        # Add members for D2 ↔ D4 VLANs
        for i, (vlan_id, eth_port) in enumerate(zip(self.data.dut2_d4_vlans, self.data.dut2_d4_eth_ports)):
            self._add_member_to_vlan(dut2, vlan_id, eth_port)

        for i, (vlan_id, eth_port) in enumerate(zip(self.data.dut4_d2_vlans, self.data.dut4_d2_eth_ports)):
            self._add_member_to_vlan(dut4, vlan_id, eth_port)

        # Add members for D4 ↔ D3 VLANs
        for i, (vlan_id, eth_port) in enumerate(zip(self.data.dut4_d3_vlans, self.data.dut4_d3_eth_ports)):
            self._add_member_to_vlan(dut4, vlan_id, eth_port)

        for i, (vlan_id, eth_port) in enumerate(zip(self.data.dut3_d4_vlans, self.data.dut3_d4_eth_ports)):
            self._add_member_to_vlan(dut3, vlan_id, eth_port)

        st.log("All Ethernet members added to VLANs as tagged members")
        time.sleep(WAIT_AFTER_VLAN_CONFIG)
        ...
        st.log("\n" + "-" * 80)
        st.log("STEP 4: Configure IP addresses on all 12 VLAN interfaces")
        st.log("-" * 80)

        # Configure IPs on D1 ↔ D2 VLANs
        for i, (vlan_id, ip) in enumerate(zip(self.data.dut1_d2_vlans, self.data.dut1_d2_ips)):
            self._configure_vlan_ip(dut1, vlan_id, ip)

        for i, (vlan_id, ip) in enumerate(zip(self.data.dut2_d1_vlans, self.data.dut2_d1_ips)):
            self._configure_vlan_ip(dut2, vlan_id, ip)

        # Configure IPs on D2 ↔ D4 VLANs
        for i, (vlan_id, ip) in enumerate(zip(self.data.dut2_d4_vlans, self.data.dut2_d4_ips)):
            self._configure_vlan_ip(dut2, vlan_id, ip)

        for i, (vlan_id, ip) in enumerate(zip(self.data.dut4_d2_vlans, self.data.dut4_d2_ips)):
            self._configure_vlan_ip(dut4, vlan_id, ip)

        # Configure IPs on D4 ↔ D3 VLANs
        for i, (vlan_id, ip) in enumerate(zip(self.data.dut4_d3_vlans, self.data.dut4_d3_ips)):
            self._configure_vlan_ip(dut4, vlan_id, ip)

        for i, (vlan_id, ip) in enumerate(zip(self.data.dut3_d4_vlans, self.data.dut3_d4_ips)):
            self._configure_vlan_ip(dut3, vlan_id, ip)

        st.log("IP addresses configured on all 12 VLAN interfaces")
        time.sleep(WAIT_AFTER_IP_CONFIG)

        st.log("PASS: IP address configuration on VLANs completed")
        ...
        st.log("\n" + "-" * 80)
        st.log("STEP 6: Verify OSPF neighbors form on all VLANs (Full state)")
        st.log("-" * 80)
        ...
        # Verify D1 VLAN interfaces
        for vlan_id in self.data.dut1_d2_vlans:
            vlan_name = f"Vlan{vlan_id}"
            if not self._verify_ospf_interface_present(d1_ospf_intf_output, vlan_name):
                error_msg = f"STEP 7: {vlan_name} not present or not up on {dut1}"
                st.error(error_msg)
                validation_failures.append(error_msg)

        # Verify D2 VLAN interfaces
        for vlan_id in self.data.dut2_d1_vlans + self.data.dut2_d4_vlans:
            vlan_name = f"Vlan{vlan_id}"
            if not self._verify_ospf_interface_present(d2_ospf_intf_output, vlan_name):
                error_msg = f"STEP 7: {vlan_name} not present or not up on {dut2}"
                st.error(error_msg)
                validation_failures.append(error_msg)

        # Verify D3 VLAN interfaces
        for vlan_id in self.data.dut3_d4_vlans:
            vlan_name = f"Vlan{vlan_id}"
            if not self._verify_ospf_interface_present(d3_ospf_intf_output, vlan_name):
                error_msg = f"STEP 7: {vlan_name} not present or not up on {dut3}"
                st.error(error_msg)
                validation_failures.append(error_msg)

        # Verify D4 VLAN interfaces
        for vlan_id in self.data.dut4_d2_vlans + self.data.dut4_d3_vlans:
            vlan_name = f"Vlan{vlan_id}"
            if not self._verify_ospf_interface_present(d4_ospf_intf_output, vlan_name):
                error_msg = f"STEP 7: {vlan_name} not present or not up on {dut4}"
                st.error(error_msg)
                validation_failures.append(error_msg)

        if len([f for f in validation_failures if "STEP 7" in f]) == 0:
            st.log("PASS: All 12 OSPF VLAN interfaces are up and operational")
        ...
        st.log("IP addresses removed from all VLANs")

        time.sleep(WAIT_AFTER_IP_CONFIG)

        # Remove Ethernet members from VLANs
        for i, (vlan_id, eth_port) in enumerate(zip(self.data.dut1_d2_vlans, self.data.dut1_d2_eth_ports)):
            self._remove_member_from_vlan(dut1, eth_port, vlan_id)

        for i, (vlan_id, eth_port) in enumerate(zip(self.data.dut2_d1_vlans, self.data.dut2_d1_eth_ports)):
            self._remove_member_from_vlan(dut2, eth_port, vlan_id)

        for i, (vlan_id, eth_port) in enumerate(zip(self.data.dut2_d4_vlans, self.data.dut2_d4_eth_ports)):
            self._remove_member_from_vlan(dut2, eth_port, vlan_id)

        for i, (vlan_id, eth_port) in enumerate(zip(self.data.dut4_d2_vlans, self.data.dut4_d2_eth_ports)):
            self._remove_member_from_vlan(dut4, eth_port, vlan_id)

        for i, (vlan_id, eth_port) in enumerate(zip(self.data.dut4_d3_vlans, self.data.dut4_d3_eth_ports)):
            self._remove_member_from_vlan(dut4, eth_port, vlan_id)

        for i, (vlan_id, eth_port) in enumerate(zip(self.data.dut3_d4_vlans, self.data.dut3_d4_eth_ports)):
            self._remove_member_from_vlan(dut3, eth_port, vlan_id)

        st.log("Ethernet members removed from all VLANs")
        ...
        # ===== TEST COMPLETE =====
        st.log("\n" + "=" * 80)
        st.log("TEST COMPLETE: OSPF Scalability Over VLAN")
        st.log("=" * 80)
        st.log("Summary:")
        st.log("  ✓ IPs removed from all Ethernet interfaces")
        st.log("  ✓ 12 VLANs created successfully")
        st.log("  ✓ Ethernet members added to VLANs (1 port per VLAN as tagged member)")
        st.log("  ✓ IP addresses configured on all 12 VLAN interfaces")
        ...
        if validation_failures:
            ...
            for dut in [dut1, dut2, dut3, dut4]:
                try:
                    st.generate_tech_support(dut=dut, name="ospf_scalability_vlan_validation_failure")
