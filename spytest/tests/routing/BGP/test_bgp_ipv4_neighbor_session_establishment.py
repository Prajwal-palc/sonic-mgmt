"""
BGP IPv4 NEIGHBOR SESSION ESTABLISHMENT - Test ID 2.4.1
Author: QA Team
...
How to run:
  ...
  --logs-path ./logs/test_bgp_ipv4_neighbor_session_establishment_$(date +%F_%H%M%S) \
  --log-level debug  --skip-init-config  --ifname-type native

Description:
  Comprehensive validation of IPv4 BGP neighbor session establishment across multiple
  ...
  'show ip bgp summary', and ensures complete cleanup. Supports both click and klish
...
Pre-requisites:
  ...
  - Required test variables (YAML): vars_bgp_ipv4_neighbor_session_establishment.yaml
...
def _candidate_var_files() -> List[Path]:
    ...
    candidates.append(project_root / "vars" / "routing" / "bgp" / "vars_bgp_ipv4_neighbor_session_establishment.yaml")
    candidates.append(Path(__file__).resolve().with_name("vars_bgp_ipv4_neighbor_session_establishment.yaml"))
    return candidates
...
def _load_yaml_data() -> SpyTestDict:
    ...
    st.report_fail("msg", f"BGP 2.4.1 variable file not found. Checked: {attempted_paths}")
    return SpyTestDict()
...
class TestBgpIpv4NeighborSessionEstablishment:
    """Testcases for validating IPv4 BGP neighbor session establishment - Test ID 2.4.1."""
    ...
    def _configure_bgp_neighbor(self, config: SpyTestDict) -> None:
        ...
        st.log(f"Configured BGP neighbor {neighbor_ip} (remote-as {remote_asn}) on {dut}")

    def _configure_bgp_neighbor_interface(self, config: SpyTestDict) -> None:
        """Configure BGP neighbor using interface (unnumbered)."""
        dut = self._resolve_dut(config.get("dut"))
        if not dut:
            st.report_fail("msg", f"Invalid DUT alias in BGP neighbor config: {config}")
        ...
        if not result:
            st.report_fail("msg", f"Failed to configure BGP neighbor on interface {interface} on {dut}")

        self._configured_neighbors.append(config)
        st.log(f"Configured BGP unnumbered neighbor on {interface} (remote-as {remote_asn}) on {dut}")
    ...
    def _verify_bgp_session_established(self, config: SpyTestDict) -> None:
        ...
        if not st.poll_wait(_check_established, self.data.verify_timeout):
            st.report_fail("msg", f"BGP session with {neighbor} did not reach Established state on {dut}")

        st.log(f"BGP session with {neighbor} is Established on {dut}")
    ...
    def _execute_testcase(self, tcid: str, cli_type: str) -> None:
        ...
        # Configure BGP neighbors (IP-based)
        ...
        st.wait(10, "Waiting for BGP sessions to stabilize")
    ...
    @pytest.mark.inventory(feature="Regression", testcases=["BGP_2.4.1.1"])
    def test_ibgp_ipv4_numbered_loopback(self) -> None:
        """Test ID 2.4.1.1 - iBGP IPv4 Numbered (Loopback-Based)."""
        for cli_type in self.data.cli_types:
            st.log(f"Running test_ibgp_ipv4_numbered_loopback with cli_type={cli_type}")
            self._execute_testcase("2.4.1.1", cli_type)
            # Cleanup for next iteration
            self.teardown_method()
            self.setup_method()
        st.report_pass("test_case_passed")

    @pytest.mark.inventory(feature="Regression", testcases=["BGP_2.4.1.2"])
    def test_ibgp_ipv4_unnumbered_loopback(self) -> None:
        """Test ID 2.4.1.2 - iBGP IPv4 Unnumbered (Loopback-Based)."""
        for cli_type in self.data.cli_types:
            st.log(f"Running test_ibgp_ipv4_unnumbered_loopback with cli_type={cli_type}")
            self._execute_testcase("2.4.1.2", cli_type)
            self.teardown_method()
            self.setup_method()
        st.report_pass("test_case_passed")

    @pytest.mark.inventory(feature="Regression", testcases=["BGP_2.4.1.3"])
    def test_ebgp_ipv4_numbered_loopback(self) -> None:
        """Test ID 2.4.1.3 - eBGP IPv4 Numbered (Loopback-Based)."""
        for cli_type in self.data.cli_types:
            st.log(f"Running test_ebgp_ipv4_numbered_loopback with cli_type={cli_type}")
            self._execute_testcase("2.4.1.3", cli_type)
            self.teardown_method()
            self.setup_method()
        st.report_pass("test_case_passed")

    @pytest.mark.inventory(feature="Regression", testcases=["BGP_2.4.1.4"])
    def test_ebgp_ipv4_unnumbered_loopback(self) -> None:
        """Test ID 2.4.1.4 - eBGP IPv4 Unnumbered (Loopback-Based)."""
        for cli_type in self.data.cli_types:
            st.log(f"Running test_ebgp_ipv4_unnumbered_loopback with cli_type={cli_type}")
            self._execute_testcase("2.4.1.4", cli_type)
            self.teardown_method()
            self.setup_method()
        st.report_pass("test_case_passed")

    @pytest.mark.inventory(feature="Regression", testcases=["BGP_2.4.1.5"])
    def test_ibgp_ipv4_numbered_direct(self) -> None:
        """Test ID 2.4.1.5 - iBGP IPv4 Numbered (Direct Back-to-Back)."""
        for cli_type in self.data.cli_types:
            st.log(f"Running test_ibgp_ipv4_numbered_direct with cli_type={cli_type}")
            self._execute_testcase("2.4.1.5", cli_type)
            self.teardown_method()
            self.setup_method()
        st.report_pass("test_case_passed")

    @pytest.mark.inventory(feature="Regression", testcases=["BGP_2.4.1.6"])
    def test_ibgp_ipv4_unnumbered_direct(self) -> None:
        """Test ID 2.4.1.6 - iBGP IPv4 Unnumbered (Direct Back-to-Back)."""
        for cli_type in self.data.cli_types:
            st.log(f"Running test_ibgp_ipv4_unnumbered_direct with cli_type={cli_type}")
            self._execute_testcase("2.4.1.6", cli_type)
            self.teardown_method()
            self.setup_method()
        st.report_pass("test_case_passed")

    @pytest.mark.inventory(feature="Regression", testcases=["BGP_2.4.1.7"])
    def test_ebgp_ipv4_numbered_direct(self) -> None:
        """Test ID 2.4.1.7 - eBGP IPv4 Numbered (Direct Back-to-Back)."""
        for cli_type in self.data.cli_types:
            st.log(f"Running test_ebgp_ipv4_numbered_direct with cli_type={cli_type}")
            self._execute_testcase("2.4.1.7", cli_type)
            self.teardown_method()
            self.setup_method()
        st.report_pass("test_case_passed")

    @pytest.mark.inventory(feature="Regression", testcases=["BGP_2.4.1.8"])
    def test_ebgp_ipv4_unnumbered_direct(self) -> None:
        """Test ID 2.4.1.8 - eBGP IPv4 Unnumbered (Direct Back-to-Back)."""
        for cli_type in self.data.cli_types:
            st.log(f"Running test_ebgp_ipv4_unnumbered_direct with cli_type={cli_type}")
            self._execute_testcase("2.4.1.8", cli_type)
            self.teardown_method()
            self.setup_method()
        st.report_pass("test_case_passed")
